...
mod tests {
    ...
    fn test_hashmap() {
        ...
        let transferred = value.relocated(affinities[0], affinities[1]);
        ...
        let transferred_empty = empty_value.relocated(affinities[0], affinities[1]);
    ...
    fn test_tuples() {
        ...
        let transferred_option = with_option.relocated(affinities[0], affinities[1]);
        ...
        let transferred_twelve = twelve.relocated(affinities[0], affinities[1]);
    ...
    fn test_function_pointers() {
        ...
        let transferred_no_args = fn_ptr_no_args.relocated(affinities[0], affinities[1]);
        ...
        let transferred_one = fn_ptr_one.relocated(affinities[0], affinities[1]);
        assert_eq!(transferred_one(5), 10);
        assert_eq!(fn_ptr_one(42), transferred_one(42));

        // Test fn(A, B) -> R
        let fn_ptr_two: fn(i32, i32) -> i32 = two_args;
        let transferred_two = fn_ptr_two.relocated(affinities[0], affinities[1]);
        ...
        let transferred_three = fn_ptr_three.relocated(affinities[0], affinities[1]);
        ...
        let transferred_many = fn_ptr_many.relocated(affinities[0], affinities[1]);
        ...
        let transferred_string = fn_string.relocated(affinities[0], affinities[1]);
        assert_eq!(transferred_string(), "hello".to_string());

        let fn_bool: fn(i32) -> bool = returns_bool;
        let transferred_bool = fn_bool.relocated(affinities[0], affinities[1]);
    ...
    fn test_result() {
        ...
        let transferred_ok = ok_value.relocated(affinities[0], affinities[1]);
        assert_eq!(transferred_ok, Ok("success".to_string()));

        // Test Err variant
        let err_value: Result<String, i32> = Err(42);
        let transferred_err = err_value.relocated(affinities[0], affinities[1]);
        assert_eq!(transferred_err, Err(42));

        // Test with complex types
        let ok_vec: Result<Vec<i32>, String> = Ok(vec![1, 2, 3]);
        let transferred_ok_vec = ok_vec.relocated(affinities[0], affinities[1]);
        assert_eq!(transferred_ok_vec, Ok(vec![1, 2, 3]));

        let err_string: Result<Vec<i32>, String> = Err("error".to_string());
        let transferred_err_string = err_string.relocated(affinities[0], affinities[1]);
    ...
    fn test_arc() {
        ...
        let transferred = arc_value.relocated(affinities[0], affinities[1]);
        ...
        let transferred_string = arc_string.relocated(affinities[0], affinities[1]);
        assert_eq!(*transferred_string, "hello".to_string());

        // Test Arc with Vec
        let arc_vec = Arc::new(vec![1, 2, 3]);
        let transferred_vec = arc_vec.relocated(affinities[0], affinities[1]);
