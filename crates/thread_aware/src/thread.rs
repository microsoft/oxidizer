...
impl ThreadBuilder {
    ...
    ///
    /// A [`ThreadId`] is never reused, even after its thread exits, so a
    /// coordinate built for a finished thread cannot alias a live worker's
    /// partition. Published per-thread values remain until their storage is
    /// dropped.
    #[must_use]
