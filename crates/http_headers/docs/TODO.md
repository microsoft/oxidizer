...
**Guideline:** [C-CONV](https://rust-lang.github.io/api-guidelines/naming.html#c-conv) — should use `as_` for free borrowed projections and `to_` for expensive conversions, explicitly including borrowed UTF-8 validation · **Confidence:** High · **Scope:** 3 checked accessors, exhaustive for `FieldValueRef`, `UserAgentView` and `ServerView`; sampled across the wider conversion API
...
projections of a retained string. `FieldValueRef` already provides `to_str`,
but documents it as an alias for `as_str`. The two opaque header views expose
the checked operation as `as_str` while internally calling `to_str`.

- `crates/http_headers/src/field_value.rs:765` — `as_str` calls `str::from_utf8(self.bytes)` at line 766; the reverse alias documentation starts at line 769
...
**Done when:** all three types offer and teach the checked `to_str` operation,
