...
#[test]
fn repeated_negotiation_errors_preserve_their_error_details() {
    let mut map = HeaderMap::new();
    map.append(http::header::ACCEPT, HeaderValue::from_static("text/plain"));
    map.append(http::header::ACCEPT, HeaderValue::from_static("\"unterminated"));

    let error = Accept::owned(&map).expect_err("an unterminated quote must be rejected");
    assert_eq!(error.kind(), DecodeErrorKind::UnterminatedQuote);
    assert_eq!(error.value_index(), Some(1));

    map.insert(http::header::ACCEPT, HeaderValue::from_static("invalid"));
    let error = Accept::owned(&map).expect_err("an invalid media range must be rejected");
