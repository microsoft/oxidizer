...
use http_headers::sink::{EncodedValues, FieldSink, FieldSinkExt, InsertError, InsertErrorKind};
use http_headers::source::{FieldLines, FieldSource};
use http_headers::{FieldName, FieldValue};

const STORAGE_ERROR: InsertError = InsertError::new(InsertErrorKind::CapacityExceeded);
...
fn response_methods_cover_success_and_failure_in_the_same_downstream_sink() {
    ...
    sink.reject = true;
    assert_eq!(sink.set_content_length(42).err(), Some(STORAGE_ERROR));
    assert_eq!(sink.values[0], "42");
    ...
    sink.reject = true;
    assert_eq!(sink.set_cache_control(CacheControl::no_cache()).err(), Some(STORAGE_ERROR));
    assert_eq!(sink.values[0], "private");
    ...
    sink.reject = true;
    assert_eq!(sink.set_access_control_max_age(300).err(), Some(STORAGE_ERROR));
    assert_eq!(
    ...
    assert_eq!(sink.append_set_cookie(second).err(), Some(STORAGE_ERROR));
