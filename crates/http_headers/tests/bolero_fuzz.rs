...
fn shaped_field_value(input: &[u8]) -> HeaderValue {
    let bytes: Vec<_> = input.iter().copied().take(MAX_VALUE_LENGTH).map(valid_field_byte).collect();
    HeaderValue::from_bytes(&bytes).expect("the generator emits only valid field-value bytes")
}

fn shaped_field_values(input: &[u8]) -> Vec<HeaderValue> {
...
fn map_with_values<H: Field>(values: &[HeaderValue]) -> HeaderMap {
    let mut map = HeaderMap::new();
    for value in values {
        map.append(H::name().as_str(), value.clone());
    }
    map
}
...
fn inserted_bytes<H: Field>(header: H::Owned) -> Vec<Vec<u8>> {
    ...
    map.get_all(H::name().as_str())
        .iter()
        .map(|value| value.as_bytes().to_vec())
        .collect()
}

fn borrowed_outcome<H: Field>(map: &HeaderMap) -> ParseOutcome {
    match H::view(map) {
...
fn owned_outcome<H: Field>(map: &HeaderMap) -> ParseOutcome {
    match H::owned(map) {
...
fn content_type_view_outcome(map: &HeaderMap) -> ContentTypeOutcome {
...
fn content_type_owned_outcome(map: &HeaderMap) -> ContentTypeOutcome {
...
fn assert_parser_consistency<H: Field>(values: &[HeaderValue]) {
    ...
    if let ParseOutcome::Parsed(encoded) = owned {
        let round_trip_values: Vec<_> = encoded
            .iter()
            .map(|bytes| HeaderValue::from_bytes(bytes).expect("a header must encode valid HeaderValue bytes"))
            .collect();
...
fn assert_duplicate_singleton_rejected<H: Field>(value: HeaderValue) {
...
fn assert_absence_differs_from_empty<H: Field>() {
    assert_eq!(owned_outcome::<H>(&HeaderMap::new()), ParseOutcome::Parsed(Vec::new()));
    ...
    H: Field,
{
    ...
    let values: Vec<_> = expected
        .iter()
        .map(|bytes| HeaderValue::from_bytes(bytes).expect("typed encoding must produce HeaderValue bytes"))
        .collect();
...
fn reference_delimited(values: &[HeaderValue], delimiter: u8) -> Result<Vec<Vec<u8>>, usize> {
...
fn delimited_outcome<H: Field>(values: &[HeaderValue]) -> Result<Vec<Vec<u8>>, (DecodeErrorKind, Option<usize>)>
where
...
fn comma_and_quoted_delimiters_match_reference() {
    bounded!().for_each(|input: &[u8]| {
        let values = shaped_field_values(input);
        let comma = reference_delimited(&values, b',').map_err(|index| (DecodeErrorKind::UnterminatedQuote, Some(index)));
        assert_eq!(delimited_outcome::<CommaItems>(&values), comma);

        let semicolon = reference_delimited(&values, b';').map_err(|index| (DecodeErrorKind::UnterminatedQuote, Some(index)));
...
fn protocol_regression_seeds() {
    let bearer_map = map_with_values::<Authorization<Bearer>>(&[HeaderValue::from_static("Bearer mF_9.B5f-4.1JqM")]);
    ...
    let basic_map = map_with_values::<Authorization<Basic>>(&[HeaderValue::from_static("Basic QWxhZGRpbjpvcGVuIHNlc2FtZQ==")]);
    ...
    let cache_map = map_with_values::<CacheControl>(&[HeaderValue::from_static("no-cache, private, max-age=60, x-note=\"a,b\"")]);
    ...
    let length_map = map_with_values::<ContentLength>(&[HeaderValue::from_static("42, 42")]);
    assert_eq!(ContentLength::view(&length_map), Ok(Some(ContentLengthOwned::new(42))));

    let content_type_map = map_with_values::<ContentType>(&[HeaderValue::from_static("text/html; charset=utf-8; boundary=\"a,b\"")]);
    ...
    let strong_map = map_with_values::<ETag>(&[HeaderValue::from_static("\"xyzzy\"")]);
    ...
    let weak_map = map_with_values::<ETag>(&[HeaderValue::from_static("W/\"xyzzy\"")]);
    ...
    let location_map = map_with_values::<Location>(&[HeaderValue::from_static(location_wire)]);
    ...
    let cookie_values = [
        HeaderValue::from_static("session=abc; Path=/; Secure; HttpOnly"),
        HeaderValue::from_static("theme=light; SameSite=Lax"),
    ];
    ...
    assert_eq!(
        ...
        cookie_values.iter().map(HeaderValue::as_bytes).collect::<Vec<_>>()
    );

    let user_agent_wire = "ExampleBrowser/1.0 (compatible; TestBot/2.0)";
    let user_agent_map = map_with_values::<UserAgent>(&[HeaderValue::from_static(user_agent_wire)]);
    ...
    let values = [
        HeaderValue::from_static("a, \"b,c\", d"),
        HeaderValue::from_static("x=\"escaped\\\" comma, stays\"; y=z"),
    ];
    assert_eq!(
    ...
    assert_eq!(
        delimited_outcome::<CommaItems>(&values),
        reference_delimited(&values, b',').map_err(|index| { (DecodeErrorKind::UnterminatedQuote, Some(index)) })
    );

    let malformed = [HeaderValue::from_static("a, \"unterminated")];
    assert_eq!(
        delimited_outcome::<CommaItems>(&malformed),
        Err((DecodeErrorKind::UnterminatedQuote, Some(0)))
    );
    let all_empty = [HeaderValue::from_static(",, ,\t,,")];
    ...
    let obs_text = HeaderValue::from_bytes(&[0x80]).expect("obs-text is a valid field value");
    ...
    let obs_etag = HeaderValue::from_bytes(&[b'"', 0x80, b'"']).expect("obs-text entity tag is valid");
    ...
    let obs_content_type =
        HeaderValue::from_bytes(b"text/plain; note=\"\x80\"").expect("obs-text quoted parameters are valid field values");
    ...
    let obs_cache_control = HeaderValue::from_bytes(b"x-note=\"\x80\"").expect("obs-text quoted directive is a valid field value");
    assert_eq!(
        owned_outcome::<CacheControl>(&map_with_values::<CacheControl>(&[obs_cache_control])),
        ParseOutcome::Parsed(vec![b"x-note=\"\x80\"".to_vec()])
    );

    assert_duplicate_singleton_rejected::<Authorization<Bearer>>(HeaderValue::from_static("Bearer abc"));
    assert_duplicate_singleton_rejected::<Authorization<Basic>>(HeaderValue::from_static("Basic dXNlcjpwYXNz"));
    assert_duplicate_singleton_rejected::<ContentType>(HeaderValue::from_static("text/plain"));
    assert_duplicate_singleton_rejected::<ETag>(HeaderValue::from_static("\"tag\""));
    assert_duplicate_singleton_rejected::<Location>(HeaderValue::from_static("/target"));
    assert_duplicate_singleton_rejected::<UserAgent>(HeaderValue::from_static("agent/1"));

    let conflicting_lengths = [HeaderValue::from_static("31"), HeaderValue::from_static("32")];
    assert_eq!(
    ...
    assert_eq!(
        owned_outcome::<CacheControl>(&map_with_values::<CacheControl>(&[HeaderValue::from_static(""),])),
        ParseOutcome::Parsed(Vec::new())
    );
    assert_eq!(
        owned_outcome::<Location>(&map_with_values::<Location>(&[HeaderValue::from_static("",)])),
        ParseOutcome::Parsed(vec![Vec::new()])
    );

    for length in [31, 32, 33, MAX_VALUE_LENGTH] {
        let value = HeaderValue::from_bytes(&vec![b'a'; length]).expect("ASCII field value is valid");
