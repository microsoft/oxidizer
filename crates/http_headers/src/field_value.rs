...
mod tests {
    ...
    fn a_long_http_value_is_shared_rather_than_copied() {
        ...
        let mut shared = FieldValue::from(&http);
        ...
        shared.set_sensitive(true);
        let round_trip = http::HeaderValue::try_from(shared.clone()).expect("retained HTTP value is valid");
        assert!(round_trip.is_sensitive());
        assert!(
            std::ptr::eq(round_trip.as_bytes().as_ptr(), http.as_bytes().as_ptr()),
            "round-tripping a retained HTTP value must preserve its shared buffer"
        );
