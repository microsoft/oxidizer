...
fn panicking_coercion_does_not_leak_any_handle() {
    ...
    fn pinned_coercion() -> Coercion<PinnedOwner, dyn PinnedValue, impl FnOnce(*const PinnedOwner) -> *const dyn PinnedValue> {
        // SAFETY: this deliberately panicking token never returns a pointer; it
        // exercises unwind behavior while the owner is temporarily extracted.
        unsafe { Coercion::new(|_| -> *const dyn PinnedValue { panic!("pinned coercion panic") }) }
    }
    ...
    {
        let drops = StdArc::new(AtomicUsize::new(0));
        let pool = Pool::<PinnedOwner>::builder().chunk_size(1).max_chunks(1).build();
        assert!(
            catch_unwind(AssertUnwindSafe(|| {
                let pinned = pool.alloc_arc_pin(PinnedOwner {
                    value: 3,
                ...
                let _ = Arc::unsize_pin(pinned, pinned_coercion());
            }))
            .is_err()
        );
        assert_eq!(drops.load(Ordering::SeqCst), 1);
        assert_eq!(pool.len(), 0);
        drop(
            pool.try_alloc_arc_pin(PinnedOwner {
                value: 4,
                drops: StdArc::clone(&drops),
                _pin: PhantomPinned,
            })
...

    {
        ...
        assert!(
            catch_unwind(AssertUnwindSafe(|| {
                ...
                let _ = Rc::unsize_pin(pinned, pinned_coercion());
            }))
            .is_err()
        );
        assert_eq!(drops.load(Ordering::SeqCst), 1);
        assert_eq!(pool.len(), 0);
        drop(
            pool.try_alloc_rc_pin(PinnedOwner {
                value: 6,
                drops: StdArc::clone(&drops),
                _pin: PhantomPinned,
            })
