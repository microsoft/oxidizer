...
//! * **Bolero** — property-based tests (`tests/bolero_plurality.rs`).
...
/// Run the full plurality correctness suite.
#[derive(FromArgs)]
struct Args {
...
fn main() -> ExitCode {
    ...
    println!();
    println!("=== Plurality Correctness Suite ===");
    println!();
    println!("Toolchains:");
...
fn run_bolero(root: &Path, tc: &Toolchains, failures: &mut Vec<String>) {
    ...
    let mut cmd = cargo(root, &tc.nightly);
    cmd.args(["test", "-p", PACKAGE, "--test", "bolero_plurality", "--all-features", "--locked"]);
