...
/// An *oversized* shared chunk (payload beyond the largest size class) is
...
#[cfg(feature = "stats")]
#[test]
fn total_bytes_allocated_matches_oversized_shared_footprint() {
    let (rec, log) = recorder();
    let arena = Arena::new_in(rec);
    ...
    let real_shared_bytes: usize = shared_chunk_sizes(&log).iter().sum();
    assert!(
        real_shared_bytes > 70_001,
        "expected an oversized shared-chunk allocation, got {:?}",
        log.lock().unwrap()
    );
    // The real (recorded) allocation size is value-aligned (rounded up).
    assert_eq!(real_shared_bytes % 8, 0, "the oversized allocation size should be value-aligned");

    let tracked = arena.stats().total_bytes_allocated;
    assert_eq!(
        tracked, real_shared_bytes as u64,
        "total_bytes_allocated ({tracked}) must equal the real (rounded) oversized shared-chunk footprint ({real_shared_bytes})"
    );
}
#[test]
