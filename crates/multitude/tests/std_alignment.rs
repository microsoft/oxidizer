...
fn vec_splice() {
    ...
    let mut v2: MVec<'_, u32> = arena.alloc_vec();
    v2.extend([1, 2, 3]);
    let r2: std::vec::Vec<u32> = v2.splice(1..1, [9]).collect();
    assert!(r2.is_empty());
...
fn vec_splice_size_hint_and_reserve() {
    ...
    // The up-front `reserve(replacement.len() + kept.len())` must use the
    // *sum*, not the product. With `replacement == capacity - 2` and
    // `kept == 2`, the correct sum exactly fills the existing capacity (no
    // realloc), whereas the product would force a larger allocation. Both
    // counts are derived from the observed capacity, so this holds for any
    // growth policy.
    ...
    assert_eq!(w.capacity(), cap, "splice reserved the sum, not the product (+ -> *)");
}
