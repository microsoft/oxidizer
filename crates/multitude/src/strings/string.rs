...
impl<'a, A: Allocator + Clone> String<'a, A> {
    ...
    /// Split the string into two at the given byte index, returning the
    /// tail `[at, len)` as a new `String` in the same arena and leaving
    ...
    /// Clone the bytes in `src` (a byte-index range into `self`) and append
    /// them to the end. Mirrors [`std::string::String::extend_from_within`].
    ///
    /// # Panics
    ///
    ...
    /// Remove the `char`s in the byte range `range` from the string, returning
    /// a draining iterator over them. Mirrors [`std::string::String::drain`].
    ///
