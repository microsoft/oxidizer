...
/// payload immediately before the value (see
/// [`thin_dst`](crate::internal::thin_dst)). The allocation also holds
...
impl<T: ?Sized + Pointee, A: Allocator + Clone> Clone for Arc<T, A> {
    ...
    fn clone(&self) -> Self {
        ...
        // Relaxed is sufficient (matching `std::sync::Arc`): the new
        // handle does not need to synchronize with anything until it is
        // dropped, and the existing handle already establishes liveness.
...
impl<T: ?Sized + Pointee, A: Allocator + Clone> Drop for Arc<T, A> {
    ...
    fn drop(&mut self) {
        ...
        let strong = unsafe { thin_dst::strong_ref::<T>(self.ptr, value_align) };
        // Release on the decrement so all prior accesses by this thread
        // happen-before the final teardown (matching `std::sync::Arc`).
        if strong.fetch_sub(1, Ordering::Release) != 1 {
            return;
        }
        // This was the last strong reference. Acquire-fence so every
        // other handle's writes are visible before we drop the value and
        // release the chunk.
        fence(Ordering::Acquire);
        // Adopt the chunk's surviving +1 *before* running `T::drop`, so a
        // panic in `T::drop` still releases the chunk via `ChunkRef`'s
        // own `Drop` during unwinding (the in-chunk slot leaks, per the
        // documented panic semantics of the `alloc_arc*` family).
        //
