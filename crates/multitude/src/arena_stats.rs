...
pub struct ArenaStats {
    ...
    /// Total bytes currently held from the underlying allocator: the sum
    /// of every chunk (header + payload) the arena owns right now —
    /// active `current_*` chunks, retired chunks still kept alive (e.g.
    /// by outstanding `Arc`/`Box` handles), and chunks parked in the
    /// size-class cache.
    ///
    ...
    /// Bytes "wasted" as unused tail space (the free region between
    /// the bump cursor and the drop-entry top), summed across every
    /// chunk the arena currently holds — both the active
    /// `current_local` / `current_shared` chunks and any chunks that
    /// have been retired but not yet returned to the cache or freed
    /// back to the underlying allocator (e.g., chunks held alive by
    /// outstanding `Arc`/`Box` handles).
    ///
