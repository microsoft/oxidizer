...
impl Attempt {
    ...
    /// Creates the first attempt for a sequence of `max_attempts`.
    ///
    /// The attempt index is `0`. It is marked as last only when `max_attempts` is `1`.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let only = Attempt::first(1);
    /// assert!(only.is_first());
    /// assert!(only.is_last());
    /// ```
    #[must_use]
    pub fn first(max_attempts: u32) -> Self {
        Self::new(0, max_attempts == 1)
    }

    /// Advances to the next attempt, or returns [`None`] if `max_attempts` has been reached.
    ///
    /// # Examples
    ///
    /// ```
    /// use recoverable::Attempt;
    ///
    /// let a = Attempt::first(3);
    /// assert_eq!(a.index(), 0);
    ///
    /// let b = a.increment(3).unwrap();
    /// assert_eq!(b.index(), 1);
    ///
    /// let c = b.increment(3).unwrap();
    /// assert_eq!(c.index(), 2);
    /// assert!(c.is_last());
    ///
    /// assert!(c.increment(3).is_none());
    /// ```
    #[must_use]
    pub fn increment(self, max_attempts: u32) -> Option<Self> {
