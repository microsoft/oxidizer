...
//! Host, `Content-Type`, and `Accept` route predicates.
//!
//! Run with
//! `cargo run -p routerama --example request_predicates --features route`.
//!
//! A `#[route(...)]` declaration may add three optional string predicates that
//! generated dispatch evaluates *after* path resolution and *before* any
//! extractor runs, always in this order:
//!
//! 1. `host = "..."` compares the URI authority first and the `Host` header
//!    second, case-insensitively. A mismatch is `404 Not Found`, because a
//!    route that belongs to another authority does not exist here.
//! 2. `consumes = "..."` requires exactly one well-formed `Content-Type`
//!    whose type/subtype matches case-insensitively. Media-type parameters
//!    such as `charset` are accepted. Missing, duplicate, malformed, or
//!    non-matching values are `415 Unsupported Media Type`.
//! 3. `produces = "..."` negotiates `Accept`. Wildcards, parameters, and
//!    quality values are honored, a more specific range overrides a broader
//!    one, and a missing `Accept` accepts anything. An unacceptable or
//!    malformed value is `406 Not Acceptable`. After the handler returns, its
//!    response `Content-Type` is replaced with the produced media type without
//!    touching the concrete body.
//!
//! Acceptability is a per-route yes/no answer, not a ranking: when several
//! routes share a method and path template, the explicit `priority` argument
//! chooses between them and the first acceptable candidate wins. A client
//! steers that choice by refusing a representation (`q=0`), not by ranking it
//! lower.
//!
//! Handlers that declare no predicate emit no predicate code and perform no
//! header lookup at all, which is why `/health` below is unaffected.
