...
//! Headers, request metadata, and typed request extensions.
//!
//! Run with
//! `cargo run -p routerama --example request_metadata --features route`.
//!
//! Every parameter of a `#[router]` handler that is not a path capture and is
//! not marked `#[body]` is a request-parts extractor. This example shows the
//! three families side by side:
//!
//! - reference parameters (`&Method`, `&Uri`, `&HeaderMap`, `&Extensions`,
//!   `&RequestParts`) borrow the request head with no copy, and the borrow
//!   stays valid across the handler's `.await`;
//! - owned parameters (`Method`, `Uri`, `Version`, `HeaderMap`) are explicit
//!   clones or copies, so the cost is visible in the signature; and
//! - typed extensions use [`ExtensionRef`] for a zero-copy `&T`,
//!   [`ClonedExtension`] for an owned `T`, and reject a missing value with
//!   [`MissingExtension`], which becomes `500 Internal Server Error`.
//!
//! A custom borrowed extractor is just a `FromRequestParts<'request, S>`
//! implementation; handlers name it with `'_` and never see a generated
//! lifetime.
//!
//! [`ClonedExtension`]: routerama::route::ClonedExtension
//! [`ExtensionRef`]: routerama::route::ExtensionRef
//! [`MissingExtension`]: routerama::route::MissingExtension
