...
fn build_report(crit: &[(String, f64)], gung: &[GungEntry]) -> String {
    ...
    out.push_str("| Router | Time (criterion) | Instructions | Branch misses | Mem accesses |\n");
    out.push_str("|---|---:|---:|---:|---:|\n");
    ...
    out.push_str("| Router | Instructions | Relative to routerama_static |\n");
    out.push_str("|---|---:|---:|\n");
...
fn regenerate_router(crate_dir: &Path) -> Result<(), AppError> {
    ...
    use routerama::{Generator, HttpMethod, RouteRule};
    ...
    let rules = ROUTES.iter().map(|(name, template)| {
        ...
        RouteRule::new(*name, HttpMethod::Get, parsed)
    });
