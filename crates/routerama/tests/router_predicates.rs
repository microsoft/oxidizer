...
fn prepared_successful_and_rejected_predicate_dispatches_allocate_zero() {
    ...
    let response = {
        let _span = success.measure_thread();
    ...
    assert_eq!(response.status(), StatusCode::OK);
    assert_eq!(response.headers()[CONTENT_TYPE], "application/json");
    assert_eq!(success.total_bytes_allocated(), 0);
    ...
    for (name, host, content_type, accept, expected) in [
        ...
        let response = {
            let _span = rejection.measure_thread();
            match future.as_mut().poll(&mut context) {
                Poll::Ready(response) => std::hint::black_box(response),
        ...
        assert_eq!(response.status(), expected);
        assert_eq!(rejection.total_bytes_allocated(), 0);
    }
}
