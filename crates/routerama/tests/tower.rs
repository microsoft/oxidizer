...
#[router(state = LifecycleState, tower)]
impl LifecycleApi {
    #[route(GET, "/")]
...
async fn configured_dynamic_and_mixed_routers_compose_with_one_shared_handle() {
    let router = Arc::new(
        Plugins::router_builder()
            .add_plugin("GET", "/plugins/{name}")
            .build()
            .expect("the dynamic route is valid"),
    );
    let mut service = PluginsRouter::tower_service::<Body, _, _, _>(router, Arc::new(Plugins), state());
...
#[tokio::test(flavor = "current_thread")]
async fn generated_exact_service_clones_and_drops_arc_handles_with_its_futures() {
    let api_drops = Arc::new(AtomicUsize::new(0));
    let state_drops = Arc::new(AtomicUsize::new(0));
    let api = Arc::new(LifecycleApi {
        drops: Arc::clone(&api_drops),
    });
    let state = Arc::new(LifecycleState {
        drops: Arc::clone(&state_drops),
    });
    let mut service = LifecycleApi::tower_service::<Body, _, _>(Arc::clone(&api), Arc::clone(&state));
    ...
    assert_eq!(api_drops.load(Ordering::Relaxed), 0);
    assert_eq!(state_drops.load(Ordering::Relaxed), 0);

    drop(api);
    drop(state);
    assert_eq!(api_drops.load(Ordering::Relaxed), 1);
...
fn transport_flavor_is_send_sync_and_static_without_bounding_core_routing() {
    ...
    fn assert_exact_transport<S, B>(_service: &S)
    where
        S: tower_service::Service<Request<Body>, Response = Response<B>> + Send + Sync + 'static,
        S::Future: Send,
    ...
    assert_exact_transport(&Api::tower_service::<Body, _, _>(Arc::new(Api), Arc::new(state())));
}

#[tokio::test(flavor = "current_thread")]
async fn generated_exact_responses_preserve_frames_trailers_size_hints_and_errors() {
    let mut service = Api::tower_service::<Body, _, _>(Arc::new(Api), Arc::new(state()));
...
fn the_adapter_adds_no_allocation_and_body_erasure_costs_exactly_one() {
    ...
    let span = exact_operation.measure_thread().iterations(1);
    ...
    drop(span);
    assert_eq!(response.status(), StatusCode::NO_CONTENT);
    assert_eq!(total_bytes_allocated(&session, "tower_exact_body"), 0);
    ...
    let span = erased_operation.measure_thread().iterations(1);
    let response = match call.as_mut().poll(&mut context) {
        Poll::Ready(response) => response.expect("routing is infallible"),
        Poll::Pending => panic!("the static handler has no pending operation"),
    };
