...
impl PrimitiveApi {
    ...
    async fn static_value(&self, value: i16) -> String {
        value.to_string()
    }

    #[route(dynamic)]
    async fn dynamic_value(&self, #[capture] value: i16) -> String {
        value.to_string()
    }
}

#[tokio::test]
async fn generated_router_uses_primitive_decoding_for_static_and_dynamic_paths() {
    let router = PrimitiveApi::router_builder()
        .add_dynamic_value("GET", "/dynamic/{value}")
        .build()
        .expect("the primitive dynamic route is valid");

    for (path, expected) in [("/static/%2D%33%32%37%36%38", "-32768"), ("/dynamic/%2B%34%32", "42")] {
        let response = router.route(&PrimitiveApi, request(Method::GET, path, ()), &()).await;
        assert_eq!(body_bytes(response.into_body()).await, expected.as_bytes());
    }

    for path in ["/static/%33%32%37%36%38", "/dynamic/%FF"] {
        let response = router.route(&PrimitiveApi, request(Method::GET, path, ()), &()).await;
        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
    }
}
