...
use routerama::route::mount::{ErasedMountRouter, ErasedMountService, MountedCaptureError, MountedRequest, MountedService};
...
fn configured_mounts_cost_static_requests_no_allocations_or_erased_calls() {
    ...
    let static_span = static_operation.measure_thread();
    ...
    assert_eq!(static_operation.total_bytes_allocated(), 0);
    assert_eq!(state.generated_calls.get(), 1);
    assert_eq!(state.mounted_calls.get(), 0);
    ...
    let mut future = pin!(StaticApi.route_with_erased_mounts(request, &state, &mounts));
    let mounted_span = mounted_operation.measure_thread();
    let response = match future.as_mut().poll(&mut context) {
        Poll::Ready(response) => response,
    ...
    drop(mounted_span);
    assert_eq!(response.status(), StatusCode::NO_CONTENT);
    assert!(mounted_operation.total_bytes_allocated() > 0);
    assert_eq!(state.mounted_calls.get(), 1);
    ...
    let miss_span = miss_operation.measure_thread();
    let response = match future.as_mut().poll(&mut context) {
        Poll::Ready(response) => response,
