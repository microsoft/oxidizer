...
// Shared runtime-mount scenarios. Seven deliberately separate comparisons:
//
// * `static_hit` proves what merely configuring erased mounts costs a generated
//   static request: the identical request is routed through `route` and through
//   `route_with_erased_mounts` over a populated mount table.
// * `dynamic_dispatch` compares a generated configured-dynamic handler against
//   an erased mounted service reached through the same entry, the same table,
//   and the same response.
// * `standalone` drives `ErasedMountRouter::route` directly, so a mounted
//   literal hit and a complete mount-table miss are visible without any
//   generated wrapper in front of them.
// * `captures` walks the four-capture inline boundary: zero, one, four, and
//   five captures through the same table and the same response.
// * `streaming` compares a concrete streaming mounted body that completes
//   against one that fails on its second frame.
// * `depth` walks the sixteen-segment matcher scratch boundary with a table
//   deep enough to disable the fixed-size fast path.
// * `table_size` scales the mount table itself to 16, 128, and 1,024 entries
//   with first, middle, last, and miss requests.
//
// These are Routerama-internal controls. A mounted service selects a different,
// type-erased capability boundary on purpose, so its numbers must never be
// folded into the generated-static five-framework comparisons.
//
// Route tables, mount tables, services, request paths, and requests are built
// outside the measured region; `run_prepared` contains exactly the routing call
// and the complete response observation.
//
// Allocations are measured in two consecutive spans per scenario. The `route`
// span covers matching, the erased call, and response construction; the `body`
// span covers complete response-body observation. `Decomposition` names which
// allocation each count is, and `assert_allocation_decomposition` checks the
// names against the measured spans. Every named category is additionally
// pinned by a differential pair that varies exactly one thing:
// `standalone/complete_miss` (no service call) isolates the boxed response
// body, `captures/four` against `captures/five` isolates the capture scratch
// spill, `depth/segments_16` against `depth/segments_17` isolates the matcher
// offset scratch spill, and `streaming/success` against `streaming/error`
// isolates the boxed body error.
...
impl Phases for Measured {
    fn routing<T>(&self, run: impl FnOnce() -> T) -> T {
        let _span = self.routing.measure_thread();
        run()
    }

    fn observing<T>(&self, run: impl FnOnce() -> T) -> T {
        let _span = self.observing.measure_thread();
        run()
    }
}
