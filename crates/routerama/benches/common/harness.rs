...
fn routerama_static_lookups() {
    for path in LOOKUPS {
        let matched = Route::resolve("GET", black_box(path));
...
#[cfg(feature = "dynamic")]
fn build_routerama_dynamic() -> ::routerama::DynRouter {
    use ::routerama::{DynRouter, HttpMethod, RouteRule};
    DynRouter::new(
        ROUTES
            .iter()
            .map(|(name, template)| RouteRule::new(*name, HttpMethod::Get, template.parse().expect("valid template"))),
    )
}

#[cfg(feature = "dynamic")]
fn routerama_dynamic_lookups(router: &::routerama::DynRouter) {
    use ::routerama::Router as _;
    for path in LOOKUPS {
