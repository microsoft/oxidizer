...
// Shared, network-free bounded `application/x-www-form-urlencoded` extraction
// fixtures. Every framework receives the same schema, the same encoded bytes,
// the same 64-byte limit, the same required media type, the same response
// payloads, and the same application-level rejection policy. Runtime and
// application construction, request and payload creation, and equivalence
// checks stay outside measured calls; buffering, decoding, handler work, and
// complete response observation stay inside.
//
// The single fixture policy every framework's application code implements is:
//
// * success                                   -> 200, marker header, `name:count:note`
// * encoded body above the limit              -> 413
// * decode failure (bad scalar, missing field) -> 400
// * missing or unsupported `Content-Type`      -> 415
//
// Deliberate exclusions and unavoidable differences are documented in
// `docs/PERF.md`; the three that shape this file are:
//
// * No invalid-UTF-8 scenario. Routerama rejects a non-UTF-8 encoded form with
//   400, while `form_urlencoded` (Axum, Actix Web, Warp) and Rocket's
//   `url_decode_lossy` substitute U+FFFD and succeed. No fixture-level policy
//   can reconcile those without replacing the framework's own decoder.
// * No repeated-key `Vec` field. `serde_urlencoded` does not support sequences,
//   so such a field would measure three frameworks against two.
// * Status normalization. Axum reports form decode failures as 422
//   (`FailedToDeserializeFormBody`) and Rocket as 422 (`UnprocessableEntity`).
//   Their handlers map their own typed error values onto the shared 400 policy,
//   exactly as the JSON fixture already does for Actix Web and Rocket.
