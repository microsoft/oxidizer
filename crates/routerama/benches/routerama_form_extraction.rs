...
//! form extraction fixtures. Rocket's split-body row is explicitly named as a
//! coalesced client body because its local client cannot retain frame
//! boundaries.
//!
//! Paired with `routerama_form_extraction_cg.rs`.
...
fn form_extraction(c: &mut Criterion) {
    ...
    for scenario in Scenario::ALL {
        let mut group = c.benchmark_group(format!("routerama_form_extraction/{}", scenario.name()));
        for framework in Framework::ALL {
            let benchmark_name = if scenario == Scenario::SplitSuccess && framework == Framework::Rocket {
...
criterion_group!(benches, form_extraction);
