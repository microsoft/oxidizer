...
#[doc(hidden)]
#[derive(Debug)]
pub struct BodyStateWitnessBody<E>(PhantomData<fn() -> E>);

impl<E> HttpBody for BodyStateWitnessBody<E> {
    type Data = Bytes;
    type Error = E;
...
///
/// The response mapping is stable for the Phase 0 contract:
///
...
impl<E> IntoResponse for BodyRejection<E> {
    ...
    fn into_response(self) -> Response<Self::Body> {
        match self {
            ...
            Self::Transport(_) | Self::InvalidUtf8(_) => StatusCode::BAD_REQUEST,
        }
        .into_response()
    }
}
    ...
    B: HttpBody<Data = Bytes>,
{
    ...
    let mut first = None;
    let mut output = None;
    let hinted_capacity = usize::try_from(minimum).unwrap_or(usize::MAX).min(LIMIT);
    ...
    while let Some(frame) = core::future::poll_fn(|context| body.as_mut().poll_frame(context)).await {
        ...
        if let Some(output) = &mut output {
            ...
            let mut combined = BytesMut::with_capacity(hinted_capacity.max(received));
