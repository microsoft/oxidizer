...
/// an opaque type: a transport adapter that must *name* one response type has
/// to erase it. Erasure is the caller's explicit choice, never an internal
/// default.
///
...
pub struct SendBoxBody {
    body: Pin<Box<dyn http_body::Body<Data = Bytes, Error = SendBoxBodyError> + Send + 'static>>,
}
...
impl http_body::Body for SendBoxBody {
    type Data = Bytes;
    type Error = SendBoxBodyError;
...
pub struct BoxBody {
    body: Pin<Box<dyn http_body::Body<Data = Bytes, Error = BoxBodyError> + 'static>>,
}
