...
[`ThreadAware`][__link5] lets that state repair itself. The runtime moves the value, then calls
[`relocate`][__link6] to report where it now lives.

## The two roles

**Library and application authors** implement [`ThreadAware`][__link7], usually through the
`#[derive(ThreadAware)]` macro. They never call [`relocate`][__link8] and
never construct a [`Place`][__link9]; the runtime does both and then invokes the implementation.
It is a callback, like [`Drop::drop`][__link10].

**Runtime authors** construct a [`Place`][__link11] per worker and call
...
shared: every thread near the same memory reports the same [`NumaNode`][__link14], which is what
...
[`Origin`][__link15] identifies the runtime that produced the place. Thread ids already distinguish
...
* State concerned only with memory locality, such as a buffer pool, keys on [`NumaNode`][__link16]
  and survives a move to another thread near the same memory.
* State owned by the runtime, such as a scheduler handle, also compares [`Origin`][__link17] and is
  released when it changes.

The ids carry no meaning beyond identity. [`Origin`][__link18] and [`NumaNode`][__link19] need not start at
...
[`Origin`][__link20] and [`NumaNode`][__link21] remain. A `no_std` library can still implement
[`ThreadAware`][__link22] and use whatever it is given; the runtime that drives relocation requires
`std` regardless.
...
Containers forward the call to what they hold: [`Option`][__link27], [`Result`][__link28], arrays, slices,
...
`BTreeSet` field is not [`ThreadAware`][__link29].
...
This crate was developed as part of <a href="https://github.com/microsoft/oxidizer">The Oxidizer Project</a>. Browse this crate's <a href="https://github.com/microsoft/oxidizer/tree/main/crates/thread_aware_core">source code</a>.
 ...
 [__cargo_doc2readme_dependencies_info]: ggGmYW0CYXZlMC43LjJhdIQb11VxC_uAPOQbtUn4Wx2-BfAbid3Nt1Y27Pobprn8Z6FjFy9hYvRhcoQb7zvbsw1jvrobSxV_DuV_JDcb-c20XleVRnobHKGP3XOUnqVhZIGCcXRocmVhZF9hd2FyZV9jb3JlZTAuMS4w
