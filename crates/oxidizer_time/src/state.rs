...
pub struct SynchronizedTimers {
    ...
    //
    // We have done a [benchmark](https://github.com/microsoft/oxidizer/blob/main/crates/oxidizer_time/benches/clock_bench.rs)
    // that compares the performance of this code by replacing the `Mutex`
    // with `RefCell`. The `RefCell` variant is around 7% faster. Practically, in real applications,
    // the difference is negligible. The real difference maker is to isolate the `Clock` to each thread.
    // This reduces the lock contention and gives linear scalability.
