...
//! glue or manual body conversion is needed. The hand-written `/healthz` route
//! also returns a neutral `HttpResponse` directly, demonstrating the
//! `IntoResponse` implementation enabled by the `axum` feature.
//!
...
fn app() -> Router {
    ...
    Router::new()
        ...
        .fallback_service(RestService::new(transcoder).with_max_body_bytes(MAX_BODY))
}

/// Axum accepts the framework-neutral response directly through its
