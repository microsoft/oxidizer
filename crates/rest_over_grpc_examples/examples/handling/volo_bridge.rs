...
//!
//! `rest_over_grpc::build` ships a built-in bridge for `tonic` (enabled by its
//! `tonic` feature): a blanket `impl Library for T where T: <tonic server
//! trait>`, so a service written once against `tonic` also serves REST. For any
//! other stack you write the bridge yourself — implement the generated
