...
// --- Base type defined at module level so both parent and child can reference it ---

struct Base;

// Composite root impls (Scheduler, Clock) generated by the Builtins macro.
Builtins!(@impls Base);

// Scoped root type: RequestContext is inserted per-request, not in the parent.
impl autoresolve::ResolveFrom<Base> for RequestContext {
    type Inputs = autoresolve::ResolutionDepsEnd;

    fn new(_: autoresolve::ResolutionDepsEnd) -> Self {
        unreachable!("scoped root type is pre-inserted into the scoped resolver")
    }
}

fn create_parent(builtins: Builtins) -> autoresolve::Resolver<Base> {
    let mut r = autoresolve::Resolver::<Base>::new_empty();
    Builtins!(@insert r, builtins);
    r
}
