...
/// The field name `scope_to_self` would look up for `expr`, or `None` when the root names no
/// field. Mirrors the match in `display::scope_to_self`, which is the only consumer of `root`.
fn root_name(expr: &Expr) -> Option<String> {
    match root(expr) {
        Root::Name(name, _) | Root::Index(name, _) => Some(name),
        Root::SelfKeyword(_) | Root::Method | Root::Unsupported => None,
    }
}
