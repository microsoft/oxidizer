...
fn entry(c: &mut Criterion) {
    ...
    let service = (Intercept::layer().on_input(|_v| {}), Execute::new(|v| async move { v })).into_service();
    group.bench_function("on-input", |b| b.iter(|| block_on(service.execute(0))));

    let service = (Intercept::layer().modify_input(|v| v + 1), Execute::new(|v| async move { v })).into_service();
    group.bench_function("modify-input", |b| b.iter(|| block_on(service.execute(0))));

    let service = (Intercept::layer().on_output(|_v| {}), Execute::new(|v| async move { v })).into_service();
    group.bench_function("on-output", |b| b.iter(|| block_on(service.execute(0))));

    let service = (Intercept::layer().modify_output(|v| v + 1), Execute::new(|v| async move { v })).into_service();
