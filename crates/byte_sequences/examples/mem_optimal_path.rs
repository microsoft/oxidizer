...
use byte_sequences::{BlockSize, CallbackMemory, HasMemory, MemoryShared, NeutralMemoryPool, Sequence, SequenceBuilder};
...
fn main() {
    ...
    let message1 = Sequence::copy_from_slice(b"Example message 1: hello, world!", &connection_memory);
    connection.write(message1.clone());

    // This message uses the neutral memory pool, which does not provide optimal memory.
    let message2 = Sequence::copy_from_slice(b"Message 2: goodbye", &neutral_memory_pool);
    ...
    let message3 = Sequence::from_sequences([message1, message2]);
...
impl Connection {
    ...
    fn write_optimal(&mut self, message: Sequence) {
    ...
    fn write_fallback(&mut self, message: Sequence) {
...
impl IoContext {
    ...
    pub fn reserve_io_memory(&self, min_len: usize, memory_configuration: MemoryConfiguration) -> SequenceBuilder {
        ...
        SequenceBuilder::from_blocks([block])
    }
}
