...
use byte_sequences::{BlockSize, FixedBlockTestMemory, Sequence};
use bytes::Buf;
...
fn entrypoint(c: &mut Criterion) {
    ...
    let ten_as_seq = Sequence::from_sequences(ten.iter().cloned());
    ...
    group.bench_function("to_bytes_single_chunk", |b| {
        let seq = Sequence::from(test_data_as_seq.clone().into_bytes());
    ...
    let mut group = c.benchmark_group("Sequence_slow");
    ...
    group.bench_function("from_many", |b| {
        b.iter_batched(
            ...
            |many_clones| {
                let _span = allocs_op.measure_thread();
                Sequence::from_sequences(black_box(many_clones))
            },
            BatchSize::SmallInput,
        );
    });
    ...
    group.bench_function("from_max_inline", |b| {
        b.iter_batched(
            ...
            |max_inline_clones| {
                let _span = allocs_op.measure_thread();
                Sequence::from_sequences(black_box(max_inline_clones))
            },
            BatchSize::SmallInput,
        );
    });
