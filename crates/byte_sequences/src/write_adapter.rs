...
/// Adapts a [`SequenceBuilder`] to implement the `std::io::Write` trait.
///
/// Instances are created via [`SequenceBuilder::as_write()`][1].
///
...
impl<'sb, 'm, M: Memory> SequenceBuilderWrite<'sb, 'm, M> {
    #[must_use]
    pub(crate) const fn new(inner: &'sb mut SequenceBuilder, memory: &'m M) -> Self {
        Self { inner, memory }
    }
}

impl<M: Memory> Write for SequenceBuilderWrite<'_, '_, M> {
    #[inline]
