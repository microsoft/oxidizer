...
fn main() {
    ...
    emit!(
        ...
        NotificationBatch {
            ...
            total_count: i64::try_from(emails.len()).expect("recipient count fits in i64"),
        }
    );
...
impl PrintingProcessor {
    ...
    fn dump(&self) {
        let captured = self.captured.lock().expect("captured fields mutex is not poisoned");
        println!("--- {} ---", self.name);
...
impl EventProcessor for PrintingProcessor {
    ...
    fn process(&self, event: &EventView<'_>) {
        let mut captured = self.captured.lock().expect("captured fields mutex is not poisoned");

        println!(
            "[{}] event={} body={}",
            self.name,
            event.name(),
            event.body().expect("the event declares a message body")
        );
