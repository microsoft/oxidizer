...
fn rule(rpc: &str, method: HttpMethod, pattern: &str) -> HttpRule {
    HttpRule::new(rpc, method, pattern.parse().expect("valid path template"))
}
...
fn binding_exposes_method_and_template() {
    let binding = Binding::new(HttpMethod::Post, "/v1/shelves/{shelf}:archive".parse().expect("valid"))
...
fn generate_router_is_standalone_and_covers_additional_bindings() {
    let rule = rule("GetShelf", HttpMethod::Get, "/v1/shelves/{shelf}")
        .add_binding(Binding::new(HttpMethod::Get, "/v1/libraries/{shelf}".parse().expect("valid")));
...
mod descriptor {
    ...
    fn generate_exposes_the_openapi_spec_per_service() {
        ...
        service.add_method(
            HttpRule::new("Get", HttpMethod::Get, "/v1/x/{id}".parse().expect("valid path template")),
            "crate::Req",
            "crate::Resp",
            None,
        );
