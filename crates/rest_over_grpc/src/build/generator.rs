...
use crate::build::generator_builder::GeneratorBuilder;
use crate::build::generator_output::GeneratedOutput;
use crate::build::service_definition::{ServiceDefinition, generate_transcoder};
...
/// Collects [`ServiceDefinition`]s and emits their generated code.
///
/// # Examples
///
/// The common `build.rs` flow — decode a descriptor and write files:
///
/// ```no_run
/// # #[cfg(feature = "build")]
...
pub struct Generator {
    ...
    pub(crate) openapi: Option<crate::build::OpenApiInfo>,
}
...
impl Generator {
    ...
    pub(crate) fn openapi_info(&self) -> Option<&crate::build::OpenApiInfo> {
        self.openapi.as_ref()
    }
    ...
    /// Renders every collected service and writes one `{module}.rest.rs` file per
    /// module into `out_dir` (services sharing a module are concatenated), plus a
    /// top-level `transcoder.rest.rs` routing across all of them.
    ///
    /// When the generator is configured with
    /// [`GeneratorBuilder::emit_openapi_spec`], it also writes a
    /// `{module}.openapi.json` document beside each descriptor-decoded service.
    ///
    /// `include!` each `.rest.rs` file into the same module as the
    /// `prost`/`pbjson` output; the generated code references message types by
    /// their package-relative Rust path. `include!` `transcoder.rest.rs` at the
    /// enclosing scope where those service modules are siblings.
    ///
    /// # Errors
    ///
...
/// # Errors
///
/// Returns a [`DescriptorError`](crate::build::DescriptorError) if the descriptor bytes
/// cannot be decoded, an annotation is malformed, or a generated file cannot be
/// written.
#[cfg(feature = "build")]
pub fn compile_fds(descriptor_set: impl AsRef<[u8]>, out_dir: impl AsRef<Path>) -> Result<(), crate::build::DescriptorError> {
    let mut generator = Generator::new();
    generator.add_all(ServiceDefinition::from_fds(
        descriptor_set.as_ref(),
        &crate::build::DescriptorOptions::new(),
    )?);
    generator.write(out_dir.as_ref()).map_err(crate::build::DescriptorError::io)?;
    Ok(())
}
...
mod tests {
    ...
    fn new_and_default_builder_share_the_defaults() {
        // `tonic` bridge on, no OpenAPI document.
    ...
    fn emit_openapi_spec_stores_the_optional_info() {
        ...
        let some = Generator::builder()
            .emit_openapi_spec(Some(crate::build::OpenApiInfo::new("Title", "v3")))
            .build();
    ...
    fn merge_openapi_docs_unions_sections_and_tolerates_missing_ones() {
        ...
        // A doc missing `paths`/`components.schemas` exercises the no-merge arms.
        let bare = r#"{"openapi":"3.1.0"}"#.to_owned();

        let merged = merge_openapi_docs(&[a, b, bare]);

        // Paths and schemas from every doc that has them are unioned.
