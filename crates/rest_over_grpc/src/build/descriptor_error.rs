...
mod tests {
    ...
    fn streaming_message_distinguishes_client_and_bidirectional_from_server_streaming() {
        // Server-streaming IS transcodable (to a streamed JSON body), so the
        // rejection must name the unsupported directions and note that server-
        // streaming is fine — a bare "is streaming" is misleading.
