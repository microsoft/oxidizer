...
use prost_reflect::{
    DescriptorPool, DynamicMessage, ExtensionDescriptor, FieldDescriptor, Kind, MessageDescriptor, MethodDescriptor, ServiceDescriptor,
    Value,
};
...
use super::descriptor_options::DescriptorOptions;
...
fn canonicalize_body_field(rpc: &str, body: &mut RequestBody, message: &MessageDescriptor) -> Result<(), DescriptorError> {
    if let RequestBody::Field(field) = body {
        require_field(rpc, field, message, "body")?.json_name().clone_into(field);
    }
    Ok(())
}
...
fn require_field(rpc: &str, field: &str, message: &MessageDescriptor, kind: &str) -> Result<FieldDescriptor, DescriptorError> {
    message
...
fn read_pattern(rpc: &str, message: &DynamicMessage) -> Result<(HttpMethod, String), DescriptorError> {
    ...
    if message.has_field_by_name("custom") {
        ...
        let method =
            HttpMethod::custom(kind).map_err(|error| DescriptorError::malformed(rpc, &format!("invalid custom HTTP method: {error}")))?;
        return Ok((method, path.to_owned()));
    }
...
mod tests {
    ...
    use super::*;
    #[cfg(feature = "build-openapi")]
    use crate::build::OpenApiInfo;
    use crate::build::generator::CodegenOptions;
    use crate::build::{Generator, compile_fds};
    ...
    /// Concatenates generated code with the default code-generation options.
    ...
    fn compile_fds_writes_module_and_transcoder_files() {
        ...
        compile_fds(&descriptor, &out).expect("descriptor compiles and writes");
    ...
    fn write_merges_openapi_specs_for_services_sharing_a_module() {
        ...
        let mut generator = Generator::builder()
            .emit_openapi_spec(Some(OpenApiInfo::new("Multi", "v1")))
            .build();
