        ...
        let schema = match route.body() {
            ...
            RequestBody::Field(field) => {
                let field_desc = input
                    .get_field_by_name(field.as_str())
                    .or_else(|| input.get_field_by_json_name(field.as_str()))
        ...
        let payload = match route.response_body() {
            ...
            ResponseBody::Field(field) => {
                let field_desc = output
                    .get_field_by_name(field.as_str())
                    .or_else(|| output.get_field_by_json_name(field.as_str()))
...
fn template_field_paths(template: &PathTemplate<'_>) -> Vec<Vec<String>> {
    template
        .segments()
        .iter()
...
mod tests {
    ...
    use super::*;
    use crate::build::{DescriptorOptions, Generator, ServiceDefinition};
    ...
    fn client_streaming_is_rejected() {
        let err = ServiceDefinition::from_fds(
            compile(
                r#"
                    syntax = "proto3";
                    package test;
    ...
    fn invalid_descriptor_is_an_error() {
        let err = ServiceDefinition::from_fds(b"not a descriptor set", &DescriptorOptions::new()).expect_err("decode fails");
