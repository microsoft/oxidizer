...
use super::emit::generate_router_with_visibility;
use super::http_rule::HttpRule;
use super::request_body::RequestBody;
use super::response_body::ResponseBody;
use super::route::Route;
use super::service_method::ServiceMethod;
...
///     "CreateShelf",
///     HttpMethod::POST,
///     PathTemplate::parse("/v1/shelves", Grammar::default()).expect("valid path template"),
/// );
///
...
pub struct ServiceDefinition {
    ...
    #[cfg(feature = "build-openapi")]
    openapi: Option<super::openapi::Builder>,
}
...
impl ServiceDefinition {
    ...
    #[cfg(feature = "build-openapi")]
    pub(crate) fn set_openapi(&mut self, openapi: super::openapi::Builder) {
    ...
    ///     "GetShelf",
    ///     HttpMethod::GET,
    ///     PathTemplate::parse("/v1/shelves/{shelf}", Grammar::default())
    ...
    ///
    /// Like [`add_method`](Self::add_method), but the generated handler yields a
    /// `ResponseStream`.
    ...
    /// # Errors
    ///
    /// Returns a [`DescriptorError`] if descriptor processing fails.
    #[cfg(feature = "build")]
    pub fn from_fds(descriptor_set: impl AsRef<[u8]>, options: &DescriptorOptions) -> Result<Vec<Self>, DescriptorError> {
        super::descriptor::definitions_from_descriptor(descriptor_set.as_ref(), options)
    }
    ...
    /// Renders this definition's service trait.
...
/// Renders the top-level `Transcoder` across all services.
pub(crate) fn generate_transcoder(services: &[ServiceDefinition]) -> TokenStream {
    ...
    let resolve = generate_router_with_visibility(&merged_routes, false);
...
/// a multi-segment reserved expansion.
fn capture_signature(template: &PathTemplate<'_>) -> (Vec<Vec<String>>, Vec<bool>) {
    let mut paths = Vec::new();
    let mut reserved = Vec::new();
    for segment in template.segments() {
        match segment {
            Segment::Variable(variable) => {
                let segments = variable.segments();
                let is_reserved = segments.len() != 1 || segments.clone().any(|segment| matches!(segment, Segment::Rest));
                paths.push(variable.field_path().split('.').map(str::to_owned).collect());
                reserved.push(is_reserved);
            }
            Segment::Affix { name, .. } => {
                paths.push(name.split('.').map(str::to_owned).collect());
                reserved.push(false);
            }
            _ => {}
        }
...
/// Assigns a distinct generated route variant to every HTTP binding.
///
/// Bindings cannot share a variant solely because their capture fields match:
/// each binding has independent request- and response-body mappings that the
/// transcode arm must preserve.
fn assign_method_variants(base: &str, method: &ServiceMethod, module: &str) -> (Vec<RouteVariant>, Vec<Route>) {
    let mut variants = Vec::new();
    let mut routes = Vec::new();
    for (index, route) in method.routes().iter().enumerate() {
        let name = if index == 0 { base.to_owned() } else { format!("{base}_b{index}") };
        let (signature, reserved_expansions) = capture_signature(&route.template());
...
fn variant_pokes(variant: &RouteVariant) -> TokenStream {
    let statements = variant
        ...
        .zip(&variant.reserved_expansions)
        .enumerate()
        .map(|(capture_index, ((path, enum_type), reserved))| {
            ...
            if let Some(enum_type) = enum_type {
                let parser = if *reserved {
                    quote! { parse_reserved_path_enum_value }
                } else {
                    quote! { parse_path_enum_value }
                };
                quote! {
                    #lvalue = ::rest_over_grpc::codegen_helpers::#parser(#capture, #enum_type::from_str_name)
                        .map_err(::rest_over_grpc::codegen_helpers::TranscodeError::into_status)?
                        .into();
                }
            } else {
                let parser = if *reserved {
                    quote! { parse_reserved_path_field }
                } else {
                    quote! { parse_path_field }
                };
                quote! {
                    #lvalue = ::rest_over_grpc::codegen_helpers::#parser(#capture)
                        .map_err(::rest_over_grpc::codegen_helpers::TranscodeError::into_status)?;
                }
            }
        });
                            ...
                            match #receiver.#fn_ident(request, &mut cx).await {
                                ::core::result::Result::Ok(stream) => {
                                    ::rest_over_grpc::transcoding::TranscodeResponse::Streaming(
                                        ::rest_over_grpc::transcoding::StreamingResponse::encode_response(
                                            stream,
                                            __stream_encoding,
                                            #response_kind,
                                        ),
...
mod tests {
    ...
    fn additional_bindings_with_different_captures_split_into_separate_arms() {
        ...
        generator.add_method(
            rule("GetShelf", HttpMethod::GET, "/v1/shelves/{shelf}")
                .add_binding(Binding::new(HttpMethod::GET, template("/v1/shelves/{shelf}/books/{book}"))),
    ...
    #[test]
    fn additional_bindings_keep_their_own_body_mappings() {
        let primary = HttpRule::new("Update", HttpMethod::PATCH, template("/v1/items/{id}"))
            .request_body(RequestBody::Whole)
            .response_body(ResponseBody::Field("item".into()))
            .add_binding(
                Binding::new(HttpMethod::POST, template("/v1/alternate/{id}"))
                    .request_body(RequestBody::Field("item".into()))
                    .response_body(ResponseBody::Whole),
            );
        let mut generator = ServiceDefinition::new("Library", None);
        generator.add_method(primary, "crate::pb::UpdateRequest", "crate::pb::UpdateResponse", None);

        let file: syn::File = syn::parse2(generator.generate(CodegenOptions::default())).expect("generated code");
        let flat: String = prettyplease::unparse(&file).split_whitespace().collect();
        let primary_start = flat.find("Route::Update_0{id:__cap0}=>").expect("primary arm");
        let additional_start = flat.find("Route::Update_0_b1{id:__cap0}=>").expect("additional arm");
        let primary_arm = &flat[primary_start..additional_start];
    ...
    #[test]
    fn multi_segment_captures_use_reserved_expansion_decoding() {
        let mut generator = ServiceDefinition::new("Library", None);
        generator.add_method(
            rule("Get", HttpMethod::GET, "/v1/{name=items/**}"),
            "crate::pb::GetRequest",
            "crate::pb::Response",
            None,
        );
