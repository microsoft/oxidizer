...
///
/// Real requests carry only a handful of query parameters, so the pairs are
/// stored inline (on the stack) and spill to the heap only past that capacity —
/// a query-bearing request therefore usually parses with no allocation. It
/// dereferences to `&[(&str, &str)]`, so it is consumed exactly like a slice
/// (and by [`decode_request`](crate::codegen_helpers::decode_request)); the
/// backing storage is a private detail and not part of the public API.
///
/// # Examples
///
/// ```
