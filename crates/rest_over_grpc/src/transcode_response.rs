...
///
/// A `StreamingResponse` forwards encoded frames as the handler produces them.
/// The HTTP status is always `200 OK`: the
...
impl StreamingResponse {
    ...
    /// mapping to every streamed message.
    #[doc(hidden)]
    #[must_use]
    pub fn encode_response<S, T>(items: S, encoding: StreamEncoding, response_body: ResponseBodyKind) -> Self
    where
        S: Stream<Item = Result<T, Status>> + Send + 'static,
        T: Serialize + 'static,
    {
