...
/// A [`Serializer`] that emits only a single top-level field of `message` as
/// JSON, writing it straight into `out` without materializing the whole
/// message as an intermediate [`serde_json::Value`] tree.
///
/// It handles the shape every message serializes as — a struct — plus the
/// transparent `Option`/newtype wrappers that flatten onto it. Any other
/// top-level shape yields [`FieldSerError::Unsupported`] so the caller can fall
/// back to the value-tree path and preserve exact behaviour for exotic types.
...
pub(crate) enum FieldSerError {
    /// The selected field is not present (or the message is not an object):
    /// callers map this to a structure error.
    Absent,
    /// The top-level shape is not a plain struct, so the caller should fall
    /// back to the value-tree path for exact parity.
    Unsupported,
    /// The message (or the selected value) failed to serialize to JSON.
    Json(serde_json::Error),
    /// A serializer-reported failure surfaced via [`serde::ser::Error::custom`].
    Custom(String),
}
...
impl<'a> Serializer for FieldSerializer<'a> {
    ...
    // A struct is the only shape that can carry the selected field directly.
    ...
    // `Option`/newtype wrappers flatten onto the inner value, mirroring how
    // `serde_json::to_value` sees through them.
    ...
    // Keyed shapes other than a plain struct are rare for messages; defer to the
    // value-tree fallback to preserve exact behaviour.
    ...
    // Every remaining shape is a scalar/sequence that cannot carry a named
    // field, so the selected field is definitively absent.
...
/// Collects a struct's fields, serializing only the one whose name matches the
/// selected field directly into the output buffer.
...
impl SerializeStruct for FieldStruct<'_> {
    ...
    fn serialize_field<T: Serialize + ?Sized>(&mut self, key: &'static str, value: &T) -> Result<(), Self::Error> {
        // Non-matching fields are dropped without serializing them at all.
...
mod tests {
    ...
    /// Asserts that calling a serializer method on a fresh [`FieldSerializer`]
    /// yields the expected [`FieldSerError`] variant (normalizing the various
    /// `Ok` types to `()`).
    ...
    fn scalar_and_sequence_shapes_report_absent() {
        // A top-level value that cannot carry a named field is definitively
        // "field absent".
    ...
    fn option_and_newtype_forward_to_the_inner_value() {
        // `Some`/newtype wrappers flatten onto the inner value; a scalar inner is
        // still "absent".
    ...
    fn keyed_non_struct_shapes_report_unsupported() {
        // Map / enum-variant shapes defer to the value-tree fallback.
    ...
    fn error_display_and_source_cover_every_variant() {
        ...
        // `serde::ser::Error::custom` maps onto the `Custom` variant.
