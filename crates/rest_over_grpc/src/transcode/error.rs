...
impl TranscodeError {
    ...
    /// A captured path variable whose decoded value did not parse into its
    /// target message field. Maps to [`Code::InvalidArgument`] like the other
    ...
    /// A captured path variable bound to an enum field whose decoded value was
    /// neither a number nor a known enum value name. Maps to
    /// [`Code::InvalidArgument`] like the other request-side failures.
    ...
    /// assert!(status.message().contains("did not match"));
    /// ```
    #[must_use]
