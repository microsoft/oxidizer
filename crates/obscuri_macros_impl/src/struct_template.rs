...
#![expect(
    ...
    reason = "Darling's macro exapansion currently uses this pattern"
)]
#![expect(
    clippy::needless_continue,
    reason = "Darling's macro exapansion triggers this lint until next version gets released (https://github.com/TedDriggs/darling/pull/402)"
)]
            ...
            .map(|&f| {
                ...
                if !opts.unredacted {
                    opts.unredacted = f
                        .attrs
                        .iter()
                        .any(|attr| attr.path().is_ident("unredacted"));
                }
                Ok(opts)
            })
...
pub fn struct_template(ident: Ident, data: &DataStruct, attrs: &[Attribute]) -> TokenStream {
    ...
    let excess_values: Vec<_> = struct_field_names
        .difference(&template_param_names)
        .collect();

    let missing_values: Vec<_> = template_param_names
        .difference(&struct_field_names)
        .collect();
    ...
    let unrestricted_params: HashSet<_> = template_params
        .iter()
    ...
    let redacted_display =
        construct_redacted_display(&template, &struct_fields, &fields, unredacted);
            ...
            TemplatePart::ParamGroup(group) => {
                ...
                group
                    ...
                    .map(|param_name| {
                        let field = field_map
                            .get(*param_name)
                            .expect("Field should exist (validated earlier)");
                        let field_ident = field.ident.as_ref().unwrap();
                        let field_type = &field.ty;

                        // Check if this specific field is marked as unredacted
                        let field_unredacted = field_opts_map
                            .get(*param_name)
                            .is_some_and(|opts| opts.unredacted);
