...
//! assert the outcome, without spinning up an async runtime. The module also
//! provides the lower-level [`poll_once`] primitive, which performs exactly one
//! poll of an unpinned future.
//!
//! # Example
//!
//! ```
...
//! ```
//!
//! # Relationship to `futures-test`
//!
//! These assertions deliberately mirror the spirit of the `assert_stream_*`
//! macros in [`futures-test`](https://docs.rs/futures-test), which offers
//! poll-once assertions for *streams* but not for plain futures.
