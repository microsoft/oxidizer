...
impl OpenOptions {
    ...
    pub async fn open(&self, dir: &Directory, path: impl AsRef<Path>) -> Result<File> {
        Ok(File::new(self.open_inner(dir, path, SharedMemory::global()).await?))
    }
    ...
    pub async fn open_with_memory(&self, dir: &Directory, path: impl AsRef<Path>, memory: impl MemoryShared) -> Result<File> {
        Ok(File::new(self.open_inner(dir, path, SharedMemory::new(memory)).await?))
    }

    /// Opens a positional file at the given path relative to the directory
    /// capability with the options specified by `self`.
    ///
    /// The returned file provides only positional I/O — all methods take
    /// `&self` and operate at explicit byte offsets.
    ///
    /// # Errors
    ///
    /// Returns the same errors as [`open`](OpenOptions::open).
    pub async fn open_positional(&self, dir: &Directory, path: impl AsRef<Path>) -> Result<crate::positional_file::PositionalFile> {
        Ok(crate::positional_file::PositionalFile::new(
            self.open_inner_positional(dir, path, SharedMemory::global()).await?,
        ))
    }
    ...
    ///
    /// Returns the same errors as [`open`](OpenOptions::open).
    pub async fn open_positional_with_memory(
        &self,
        dir: &Directory,
        path: impl AsRef<Path>,
        memory: impl MemoryShared,
    ) -> Result<crate::positional_file::PositionalFile> {
        Ok(crate::positional_file::PositionalFile::new(
            self.open_inner_positional(dir, path, SharedMemory::new(memory)).await?,
        ))
    }

    async fn open_inner(&self, dir: &Directory, path: impl AsRef<Path>, memory: SharedMemory) -> Result<FileInner> {
    ...
    async fn open_inner_positional(&self, dir: &Directory, path: impl AsRef<Path>, memory: SharedMemory) -> Result<PositionalFileInner> {
        let mut std_opts = std::fs::OpenOptions::new();
        std_opts
            .read(self.read)
            .write(self.write)
            ...
            .create(self.create)
            .create_new(self.create_new);
        PositionalFileInner::open_file(dir, path, memory, std_opts).await
    }
}
