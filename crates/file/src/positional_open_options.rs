...
use crate::positional_file::PositionalFile;
use crate::shared_memory::SharedMemory;

/// Options and flags which can be used to configure how a positional file is opened.
///
/// This builder exposes the ability to configure how a [`PositionalFile`] is opened
/// and what operations are permitted on the open file. The [`PositionalFile::open`],
/// [`PositionalFile::create`], and [`PositionalFile::create_new`] methods are aliases
/// for commonly used options using this builder.
///
/// Generally speaking, when using `PositionalOpenOptions`, you'll first call
/// [`PositionalOpenOptions::new`], then chain calls to methods to set each option,
/// then call [`PositionalOpenOptions::open`], passing a directory capability and a
/// relative path. The returned file provides only positional I/O — all methods take
/// `&self` and operate at explicit byte offsets.
#[derive(Clone, Copy, Debug)]
#[expect(clippy::struct_excessive_bools, reason = "mirrors std::fs::OpenOptions API")]
pub struct PositionalOpenOptions {
    read: bool,
    write: bool,
    append: bool,
    truncate: bool,
...
impl PositionalOpenOptions {
    ...
    /// [`create`](PositionalOpenOptions::create) method to do so.
    ...
    /// Opens a positional file at the given path relative to the directory
    /// capability with the options specified by `self`.
    ///
    /// # Errors
    ///
    ...
    pub async fn open(&self, dir: &Directory, path: impl AsRef<Path>) -> Result<PositionalFile> {
        self.open_inner(dir, path, SharedMemory::global()).await
    }

    /// Opens a positional file with the specified options using a custom memory
    /// provider.
    ///
    ...
    /// # Errors
    ///
    /// Returns the same errors as [`open`](PositionalOpenOptions::open).
    pub async fn open_with_memory(&self, dir: &Directory, path: impl AsRef<Path>, memory: impl MemoryShared) -> Result<PositionalFile> {
        self.open_inner(dir, path, SharedMemory::new(memory)).await
    }

    async fn open_inner(&self, dir: &Directory, path: impl AsRef<Path>, memory: SharedMemory) -> Result<PositionalFile> {
        ...
        let file = dir
            ...
            .await?;
        Ok(PositionalFile::new(FileInner::new(
...
impl Default for PositionalOpenOptions {
    fn default() -> Self {
        Self::new()
    }
}
