...
//!
//! 2. **Managed buffers via [`bytesbuf`].** Reads produce
...
//!
//! 3. **Capability-based access control.** All filesystem operations are scoped to a
//!    [`Directory`] capability obtained via [`Root::bind_std`]. Paths are always relative
...
//! | [`PositionalFile`]          | Positional read + write | [`PositionalFile::open`], [`PositionalFile::create`], [`OpenOptions`] |
//!
//! ```no_run
//! # async fn example() -> std::io::Result<()> {
//! use std::path::Path;
