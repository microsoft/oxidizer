...
// Skip under Miri — these tests perform real filesystem I/O which Miri cannot emulate.
#![cfg(not(miri))]
...
async fn setup() -> (TempDir, file::Directory) {
    ...
    let dir = Root::bind_std(&thunker, tmp.path()).await.unwrap();
    (tmp, dir)
}
...
mod root {
    ...
    async fn bind_to_valid_directory_succeeds() {
        let tmp = TempDir::new().unwrap();
        let thunker = Thunker::builder().build();
        let _dir = Root::bind_std(&thunker, tmp.path()).await.unwrap();
    }

    #[tokio::test]
    async fn bind_to_non_existent_path_fails() {
        let thunker = Thunker::builder().build();
        let result = Root::bind_std(&thunker, Path::new("/tmp/__nonexistent_path_12345__")).await;
    ...
    async fn bind_to_file_fails() {
        ...
        let thunker = Thunker::builder().build();
        let result = Root::bind_std(&thunker, &file_path).await;
        assert!(result.is_err());
    }
}
...
mod read_only_file {
    ...
    #[tokio::test]
    async fn read_max_into_bytesbuf() {
        let (_tmp, dir) = setup().await;
        ...
        let n = f.read_max_into_bytesbuf(5, &mut buf).await.unwrap();
        assert_eq!(n, 5);
        assert_eq!(buf.len(), 5);
    }

    #[tokio::test]
    async fn read_into_bytesbuf() {
        let (_tmp, dir) = setup().await;
        ...
        let n = f.read_into_bytesbuf(&mut buf).await.unwrap();
        assert!(n > 0);
    }
    ...
    async fn metadata_works() {
        ...
        let mut f = ReadOnlyFile::open(&dir, "md.txt").await.unwrap();
    ...
    async fn try_clone_shares_state() {
        ...
        let mut f = ReadOnlyFile::open(&dir, "clone.txt").await.unwrap();
    ...
    async fn lock_and_unlock() {
        ...
        let mut f = ReadOnlyFile::open(&dir, "lock.txt").await.unwrap();
        f.lock().await.unwrap();
    ...
    async fn lock_shared_works() {
        ...
        let mut f = ReadOnlyFile::open(&dir, "slock.txt").await.unwrap();
    ...
    async fn try_lock_and_try_lock_shared() {
        ...
        let mut f = ReadOnlyFile::open(&dir, "tlock.txt").await.unwrap();
...
mod write_only_file {
    ...
    async fn set_permissions_works() {
        let (_tmp, dir) = setup().await;
        let mut f = WriteOnlyFile::create(&dir, "perms.txt").await.unwrap();
        let md = f.metadata().await.unwrap();
    ...
    async fn set_modified_works() {
        let (_tmp, dir) = setup().await;
        let mut f = WriteOnlyFile::create(&dir, "mod.txt").await.unwrap();
    ...
    async fn set_times_works() {
        let (_tmp, dir) = setup().await;
        let mut f = WriteOnlyFile::create(&dir, "times.txt").await.unwrap();
    ...
    async fn try_clone_works() {
        let (_tmp, dir) = setup().await;
        let mut f = WriteOnlyFile::create(&dir, "tc.txt").await.unwrap();
        let _f2 = f.try_clone().await.unwrap();
    }
...
mod read_write_file {
    ...
    async fn lock_unlock_cycle() {
        let (_tmp, dir) = setup().await;
        let mut f = File::create(&dir, "rwlock.txt").await.unwrap();
        f.lock().await.unwrap();
        f.unlock().await.unwrap();
        f.lock_shared().await.unwrap();
        f.unlock().await.unwrap();
    ...
    async fn set_permissions_and_set_modified_and_set_times() {
        let (_tmp, dir) = setup().await;
        let mut f = File::create(&dir, "rwperms.txt").await.unwrap();
        let md = f.metadata().await.unwrap();
    ...
    async fn try_clone_works() {
        let (_tmp, dir) = setup().await;
        let mut f = File::create(&dir, "rwtc.txt").await.unwrap();
        let _f2 = f.try_clone().await.unwrap();
    }
}
...
mod read_only_new_api {
    ...
    #[tokio::test]
    async fn read_into_bytesbuf_at_works() {
        let (_tmp, dir) = setup().await;
        ...
        let n = f.read_into_bytesbuf_at(5, &mut buf).await.unwrap();
        assert!(n > 0);
    }

    #[tokio::test]
    async fn read_exact_into_bytesbuf_success() {
        let (_tmp, dir) = setup().await;
        dir.write_slice("reib.txt", b"hello world").await.unwrap();
        let mut f = ReadOnlyFile::open(&dir, "reib.txt").await.unwrap();
        let mem = GlobalPool::new();
        let mut buf = mem.reserve(32);
        f.read_exact_into_bytesbuf(5, &mut buf).await.unwrap();
        assert_eq!(buf.len(), 5);
    }

    #[tokio::test]
    async fn read_exact_into_bytesbuf_eof_is_error() {
        let (_tmp, dir) = setup().await;
        dir.write_slice("reibf.txt", b"hi").await.unwrap();
        let mut f = ReadOnlyFile::open(&dir, "reibf.txt").await.unwrap();
        let mem = GlobalPool::new();
        let mut buf = mem.reserve(32);
        let err = f.read_exact_into_bytesbuf(100, &mut buf).await;
        assert!(err.is_err());
    }

    #[tokio::test]
    async fn read_exact_into_bytesbuf_at_success() {
        let (_tmp, dir) = setup().await;
        dir.write_slice("reiba.txt", b"0123456789").await.unwrap();
        let f = ReadOnlyPositionalFile::open(&dir, "reiba.txt").await.unwrap();
        let mem = GlobalPool::new();
        let mut buf = mem.reserve(32);
        f.read_exact_into_bytesbuf_at(2, 4, &mut buf).await.unwrap();
        assert_eq!(buf.len(), 4);
    }

    #[tokio::test]
    async fn read_exact_into_bytesbuf_at_eof_is_error() {
        let (_tmp, dir) = setup().await;
        dir.write_slice("reibaf.txt", b"hi").await.unwrap();
        let f = ReadOnlyPositionalFile::open(&dir, "reibaf.txt").await.unwrap();
        let mem = GlobalPool::new();
        let mut buf = mem.reserve(32);
        let err = f.read_exact_into_bytesbuf_at(0, 100, &mut buf).await;
        assert!(err.is_err());
    }
...
mod read_only_positional_file {
    ...
    #[tokio::test]
    async fn read_into_slice_at_single_read() {
        let (_tmp, dir) = setup().await;
        dir.write_slice("rs.txt", b"slice_test").await.unwrap();
        let f = ReadOnlyPositionalFile::open(&dir, "rs.txt").await.unwrap();
        let mut buf = [0u8; 4];
        let n = f.read_into_slice_at(6, &mut buf).await.unwrap();
        assert_eq!(n, 4);
...
mod positional_file {
    ...
    async fn read_slice_at_and_write_slice_at() {
        ...
        let mut buf = [0u8; 4];
        let n = f.read_into_slice_at(1, &mut buf).await.unwrap();
        assert_eq!(n, 4);
