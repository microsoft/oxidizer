...
impl ThunkerBuilder {
    ...
    /// # Panics
    ///
    /// Panics if [`max_thread_count`](Self::max_thread_count) or
    /// [`channel_capacity`](Self::channel_capacity) were set to zero. Both
    /// must be at least 1: a zero thread count would prevent any work from
    /// ever running, and a zero channel capacity is rejected by the
    /// underlying bounded MPMC queue.
    #[must_use]
    pub fn build(self) -> Thunker {
        assert!(
            self.max_thread_count > 0,
            "ThunkerBuilder::max_thread_count must be at least 1; got 0"
        );
        assert!(
            self.channel_capacity > 0,
            "ThunkerBuilder::channel_capacity must be at least 1; got 0"
        );
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "max_thread_count must be at least 1")]
    fn build_rejects_zero_max_thread_count() {
        let _ = ThunkerBuilder::new().max_thread_count(0).build();
    }

    #[test]
    #[should_panic(expected = "channel_capacity must be at least 1")]
    fn build_rejects_zero_channel_capacity() {
        let _ = ThunkerBuilder::new().channel_capacity(0).build();
    }
}
