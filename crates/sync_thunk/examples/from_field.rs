...
//! Shows `#[thunk(from = me.thunker)]` — the thunker is a struct field
//! reached through an owned `Arc<Self>` parameter.
//!
//! `#[thunk]` matches `tokio::task::spawn_blocking`'s soundness model and
//! refuses receivers and reference parameters; we wrap the call in a thin
//! method that hands an `Arc<Self>` to the thunked associated function.
