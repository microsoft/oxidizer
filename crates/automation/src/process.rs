...
mod tests {
    ...
    #[test]
    #[ignore = "helper process spawned by kill_by_pid_terminates_process"]
    fn sleeping_child() {
    ...
    fn kill_by_pid_terminates_process() {
        let mut child = Command::new(env::current_exe().expect("test executable path should be available"))
            .args(["--ignored", "--exact", "process::tests::sleeping_child"])
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .expect("sleeping child should start");

        kill_by_pid(child.id());

        let deadline = Instant::now() + Duration::from_secs(5);
        let exited = loop {
            if child.try_wait().expect("sleeping child status should be available").is_some() {
                break true;
            }
        ...
        if !exited {
            child.kill().expect("sleeping child cleanup should succeed");
            child.wait().expect("sleeping child cleanup should complete");
        }

        assert!(exited, "kill_by_pid must terminate the target process");
    }
}
