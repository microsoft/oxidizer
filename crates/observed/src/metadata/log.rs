...
mod tests {
    ...
    #[test]
    fn log_description_preserves_each_field_independently() {
        let desc = LogDescription::new("http.request", Severity::Warn, Some("done"));
        // Guard against fields being aliased or transposed with one another.
        assert_eq!(desc.name(), "http.request");
        assert_eq!(desc.severity(), Severity::Warn);
        assert_eq!(desc.body(), Some("done"));

        // A description built without a body reports `None` rather than
        // echoing the name into the body slot.
        let no_body = LogDescription::new("http.request", Severity::Warn, None);
        assert_eq!(no_body.body(), None);
        assert_eq!(no_body.name(), "http.request");
    }
}
