...
fn typed_event_source_crate_is_read() {
    ...
    // typed arm that the dyn path does not.
    let saw = Arc::new(AtomicBool::new(false));
    let sink = Sink::new(
        "typed",
        vec![Arc::new(ReadAllProcessor { saw: Arc::clone(&saw) })],
        tick::SimpleClock::new_frozen(),
    );

    emit!(sink, ProbeEvent::new(1));

    assert!(saw.load(Ordering::SeqCst), "processor should have seen the typed event");
}
