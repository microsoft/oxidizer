...
use observed::{Event, Sink, emit, event};
...
fn main() {
    ...
    for entry in routing_log.lock().expect("lock is not poisoned").iter() {
        println!("  {entry}");
    }

    println!();
    println!("HttpOnlyProcessor received (prefiltered via is_interested):");
    for entry in http_log.lock().expect("lock is not poisoned").iter() {
        println!("  {entry}");
    }
    ...
    for entry in name_log.lock().expect("lock is not poisoned").iter() {
        println!("  {entry}");
    }
}
...
fn passthrough_engine() -> data_privacy::RedactionEngine {
    redaction::passthrough_redaction_engine()
}
...
/// An event with no signal annotations - neither a severity attribute nor
/// metric attributes.
...
impl TypeRoutingProcessor {
    ...
    fn new(log: Arc<Mutex<Vec<String>>>) -> Self {
        ...
        handlers.insert(
            <HttpRequest as Event>::DESCRIPTION
                .type_id()
                .expect("event description carries a TypeId"),
        ...
        handlers.insert(
            <BorrowedEvent<'_> as Event>::DESCRIPTION
                .type_id()
                .expect("event description carries a TypeId"),
            Box::new(move |event, engine| {
                let fields = collect_fields(event, engine);
        ...
        handlers.insert(
            <GenericEvent<i64> as Event>::DESCRIPTION
                .type_id()
                .expect("event description carries a TypeId"),
            Box::new(move |event, engine| {
                let fields = collect_fields(event, engine);
        ...
        handlers.insert(
            <GenericEvent<String> as Event>::DESCRIPTION
                .type_id()
                .expect("event description carries a TypeId"),
            Box::new(move |event, engine| {
                let fields = collect_fields(event, engine);
        ...
        // for logs since there is no severity annotation.
        let lifecycle_log = Arc::clone(&log);
        handlers.insert(
            <LifecycleReady as Event>::DESCRIPTION
                .type_id()
                .expect("event description carries a TypeId"),
            Box::new(move |event, engine| {
                let fields = collect_fields(event, engine);
...
impl EventProcessor for TypeRoutingProcessor {
    ...
    fn process(&self, event: &EventView<'_>) {
        ...
        let type_id = event.description().type_id().expect("event description carries a TypeId");
...
impl EventProcessor for HttpOnlyProcessor {
    fn is_interested(&self, description: &EventDescription) -> bool {
        description.type_id() == <HttpRequest as Event>::DESCRIPTION.type_id()
    }
