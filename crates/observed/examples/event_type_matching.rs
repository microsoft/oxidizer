...
impl TypeRoutingProcessor {
    ...
    fn new(log: Arc<Mutex<Vec<String>>>) -> Self {
        ...
        handlers.insert(
            <HttpRequest as Event>::DESCRIPTION.type_id().unwrap(),
        ...
        handlers.insert(
            <BorrowedEvent<'_> as Event>::DESCRIPTION.type_id().unwrap(),
            Box::new(move |event, engine| {
                let fields = collect_fields(event, engine);
                borrowed_log
                    .lock()
        ...
        handlers.insert(
            <GenericEvent<i64> as Event>::DESCRIPTION.type_id().unwrap(),
            Box::new(move |event, engine| {
                let fields = collect_fields(event, engine);
                generic_i64_log
                    .lock()
        ...
        handlers.insert(
            <GenericEvent<String> as Event>::DESCRIPTION.type_id().unwrap(),
            Box::new(move |event, engine| {
                let fields = collect_fields(event, engine);
                generic_string_log
                    .lock()
        ...
        handlers.insert(
            <LifecycleReady as Event>::DESCRIPTION.type_id().unwrap(),
            Box::new(move |event, engine| {
                let fields = collect_fields(event, engine);
                lifecycle_log
                    .lock()
