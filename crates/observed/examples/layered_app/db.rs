...
#[histogram(query_ms, name = "db.query.duration_ms")]
...
#[event("db.pool.status")]
#[debug("Connection pool status")]
#[gauge(active_connections, name = "db.pool.active_connections")]
...
#[histogram(query_ms, name = "db.slow_query.duration_ms")]
...
pub(crate) fn query_users(sink: &Sink, table_id: i64) -> i64 {
    ...
    .enrich_for(
        // The DB sink is isolated, so it only accepts entries addressed to it.
        // Targeting keeps this library-internal attribute off the host
        // application's records as well.
        sink,
        crate::DB,
        DbSystemEnrich {
