...
- An optional **event sampler**, which gates the entire event for that Sink before any processor runs.
...
Routing starts per processor: the compile-time metadata generated by `#[event(...)]` is handed to `EventProcessor::is_interested`, which determines
whether the event must be constructed and which processors are eligible to receive it. After construction and timestamping, an optional
`EventSampler` makes one decision for the leaf sink. `Drop` suppresses every processor on that sink; `Continue` preserves the normal
per-processor routing. Leaves in a composite decide independently. See the `observed::sampling` module and
