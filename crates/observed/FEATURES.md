...
- **Per-signal attributes** - struct-level `#[event("...")]` declares the canonical event name; an optional severity attribute (`#[info]`, `#[warning]`, `#[error]`, `#[debug]`, `#[trace]`, `#[fatal]`) opts the event into a log signal, and one or more struct-level instrument attributes (`#[counter]`, `#[updown_counter]`, `#[gauge]`, `#[histogram]`) opt it into metrics (at least one signal is required); log/metric names default to the event name; `#[event("...", disabled)]` marks the event as opt-in only
...
- **Value-type enforcement** - `#[counter(x)]` requires the value field to be an unsigned integer, and `#[updown_counter(x)]` requires a signed integer; a mismatch is a compile-time error
