...
- **Value-type enforcement** - `#[counter(x)]` requires the value field to be an unsigned integer, and `#[updown_counter(x)]` requires a signed integer; a metric value field may never be an `Option<T>` (record optional data as a dimension instead); a mismatch is a compile-time error
