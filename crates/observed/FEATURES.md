...
- **Per-signal attributes** - struct-level `#[event(name = "...")]` declares the canonical event name; `#[log(severity = ..., message = "...")]` and/or a `#[metric(kind = ...)]` instrument attribute declare which signals the event produces (at least one is required); log/metric names default to the event name; `#[event(name = "...", disabled)]` marks the event as opt-in only
