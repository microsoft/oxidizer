...
//! The [`Variable`] binding within a parsed path template.

use crate::segment::Segment;

/// A `{field.path=sub-template}` variable binding within a [`PathTemplate`].
///
...
/// let template = PathTemplate::parse("/v1/{book.name=books/*}")?;
...
impl Variable {
    ...
    ///
    /// # fn main() -> Result<(), http_path_template::ParseError> {
    /// let template = PathTemplate::parse("/v1/{shelf.id}")?;
    /// let Segment::Variable(variable) = &template.segments()[1] else {
    ///     panic!("expected variable")
    /// };
    /// assert_eq!(
    ...
    ///
    /// # fn main() -> Result<(), http_path_template::ParseError> {
    /// let template = PathTemplate::parse("/v1/{name=shelves/*}")?;
    /// let Segment::Variable(variable) = &template.segments()[1] else {
    ///     panic!("expected variable")
    /// };
    /// assert_eq!(
