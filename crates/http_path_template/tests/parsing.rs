...
/// not a variable.
fn variable_at(template: &PathTemplate, index: usize) -> Option<(Vec<String>, Vec<Segment>)> {
    match &template.segments()[index] {
        Segment::Variable(variable) => Some((variable.field_path().to_vec(), variable.segments().to_vec())),
        _ => None,
    }
}
...
fn from_str_matches_parse() {
    let parsed: PathTemplate = "/v1/shelves/{shelf}:get".parse().expect("valid");
    assert_eq!(
        parsed,
        PathTemplate::parse("/v1/shelves/{shelf}:get", Grammar::default()).expect("valid")
    );
    "no-leading-slash".parse::<PathTemplate>().unwrap_err();
}
...
fn parses_variable_shorthand_to_single() {
    ...
    assert_eq!(t.segments()[0], Segment::Literal("v1".into()));
    assert_eq!(t.segments()[1], Segment::Literal("shelves".into()));
...
fn parses_variable_subtemplate_and_rest() {
    ...
    assert_eq!(segments, [Segment::Literal("books".into()), Segment::Rest]);
}
...
fn variable_accessors_return_field_and_subtemplate() {
    ...
    let (field_path, segments) = variable_at(&t, 1).expect("segment is a variable");
    assert_eq!(field_path, ["shelf", "id"]);
    assert_eq!(segments, [Segment::Literal("books".into()), Segment::Single]);
}
...
fn parse_error_display_covers_every_kind() {
    let cases: [(&str, ParseErrorPredicate, &str); 11] = [
        ...
        (
            ...
            "literal segment is empty or contains a misplaced '*'",
        ),
...
fn colon_inside_braces_is_not_a_verb() {
    ...
    assert_eq!(segments, [Segment::Literal("c:d".into())]);
}
...
fn display_round_trips_through_parse() {
    ...
    for template in templates {
        ...
        let reparsed = PathTemplate::parse(parsed.to_string(), Grammar::default()).expect("rendered template is valid");
...
fn extended_parses_suffix_prefix_and_both() {
    ...
    assert_eq!(
        ...
        Segment::Affix {
            prefix: "img-".to_owned(),
            name: vec!["img".to_owned(), "id".to_owned()],
            suffix: ".png".to_owned(),
        }
    );
}
...
fn extended_affix_round_trips_through_display() {
    for template in ["/files/{name}.json", "/v{version}/x", "/img-{img.id}.png"] {
        ...
        let reparsed = PathTemplate::parse(parsed.to_string(), Grammar::default().with_segment_affixes()).expect("valid");
        assert_eq!(parsed, reparsed, "round trip for {template:?}");
    }
}
