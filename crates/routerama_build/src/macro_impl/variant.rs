...
use syn::{Fields, GenericParam, Generics, Variant};

use super::route_attr::{RouteAttr, RouteTarget};
...
pub(super) fn has_capture_lifetime(generics: &Generics) -> syn::Result<bool> {
...
#[cfg(any(feature = "resolve", feature = "route"))]
pub(super) fn routes_for_variant(variant: &Variant) -> syn::Result<Vec<Route>> {
    ...
    for route_attr in route_attrs {
        let RouteAttr { target, .. } = route_attr.parse_args()?;
        let RouteTarget::Static { method, path } = target else {
            return Err(syn::Error::new(
                route_attr.span(),
                "`#[route(dynamic)]` does not declare a compile-time method or path",
            ));
        };
...
#[cfg(any(feature = "resolve", feature = "route"))]
pub(super) fn declared_fields(variant: &Variant) -> syn::Result<Vec<String>> {
