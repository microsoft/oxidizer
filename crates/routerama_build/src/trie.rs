...
pub fn build_trie(routes: &[Route]) -> Trie {
    ...
    for (route_index, route) in routes.iter().enumerate() {
        ...
        max_segments = max_segments.max(insert_route(&mut root, route, route_index));
    }
    Trie {
        root,
        max_segments,
...
fn insert_route(root: &mut Node, route: &Route, route_index: usize) -> usize {
...
/// element-for-element with `DynMatch::captures_ordered` (and the pair order of
/// `DynMatch::captures`). A higher-level crate can therefore resolve variable
/// names to slot indices once at registration using this function, then index
/// straight into `captures_ordered()` at match time.
#[must_use]
...
mod tests {
    ...
    fn rule(name: &str, method: HttpMethod, template: &str) -> Route {
        Route::new(
            name,
            method,
    ...
    fn ext_rule(name: &str, method: HttpMethod, template: &str) -> Route {
        Route::new(
            name,
            method,
