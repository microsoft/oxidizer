...
- ⚠️ Breaking
  ...
  - `Origin` now accepts any URI scheme (not only HTTP and HTTPS). `Origin::port()` (and the delegating `BaseUri::port()`) now returns `Option<u16>`: it returns the explicit port when present, falls back to `80`/`443` for the `http`/`https` schemes, and returns `None` for other schemes without an explicit port. `Origin::try_from_parts(scheme, authority) -> Result<Self, UriError>` was replaced by the infallible `Origin::from_parts(scheme: Scheme, authority: Authority) -> Self`. `BaseUri::try_from_raw_parts` no longer rejects non-HTTP schemes (host validation still applies). Parsing an `Origin`/`BaseUri` from a string with a non-HTTP scheme now succeeds.
