...
//! Slot-level tests for the enrichment chain and cross-thread context transfer.
//!
//! These inspect `Sink::current_enrichments()` directly, so they assert on the
//! shape of the enrichment chain itself: how many entries a scope pushed, the
//! order they sit in, which sink's slot they landed on, and that a scope unwinds
//! completely. `enrichment.rs` covers the same features from the other end, by
//! emitting events and asserting the dimensions that reach a processor. Both
//! levels are needed: a chain-ordering regression is invisible in a record whose
//! dimensions are compared as a set.
//!
//! Covers DESIGN.md requirements:
//! - Scoped, automatic enrichment (stacking and unwinding)
//! - Per-sink enrichment slots and targeted enrichment
//! - Composite broadcast to every leaf
//! - Cross-thread context transfer (`Sink::transfer_context`)
use std::sync::Arc;

use data_privacy::{DataClass, Sensitive};
use observed::context::{Transfer, Transferred};
use observed::enrichment::EnrichFnExt;
use observed::processing::EventProcessor;
use observed::{Enrichment, Sink, SinkId};
...
#[cfg(not(miri))] // only used by the tokio multi-thread test below, which is disabled under miri
#[derive(Enrichment)]
struct AsyncCtx {
...
/// and `Transfer::apply_current_thread` restores each child's chain on the
...
fn composite_transfer_preserves_divergent_children() {
    ...
    (|| {
        ...
        let b_for_thread = b.clone();
        std::thread::spawn(move || {
            let _g = transfer.apply_current_thread();
...
fn empty_composite_transfer_context_is_empty() {
    ...
    let _replay = transfer.apply_current_thread();
...
fn with_enrichment_on_empty_transfer_short_circuits_for_every_push() {
    ...
    {
        let _g = after_one.apply_current_thread();
        assert!(observer.current_enrichments().is_empty());
    }
    ...
    {
        let _g = after_two.apply_current_thread();
        assert!(observer.current_enrichments().is_empty());
    }

    // The original empty transfer is itself untouched.
    {
        let _g = empty.apply_current_thread();
        assert!(observer.current_enrichments().is_empty());
    }
}
...
/// Stacking two `Transfer::apply_current_thread` calls on the same slot unwinds
...
/// Enriching a sink that is not in the composite leaves the
