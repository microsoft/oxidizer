...
//!
//! Verifies that the derive macro correctly generates the `Enrichment` trait
//! implementation, and that all field-level annotations work as expected:
...
use observed::enrichment::{EnrichFnExt, Enrichment as _, EnrichmentEntry};
use observed::{Enrichment, Severity, emit};
use observed_testing::events::ProbeEvent;
use observed_testing::types::PublicString;
use observed_testing::{ExpectedEnrichmentEntry, ExpectedEvent, TEST_ID, test_emitter};
...
// ================================================================================================
// The `Enrichment` conversion contract
// ================================================================================================

/// `Enrichment` is implementable by hand, not just by the derive: the trait and
/// its `into_entries` method are the documented conversion surface, and
/// `.enrich()` accepts any `impl Enrichment`. Useful when entries are computed
/// at runtime rather than mapped one-to-one from fields.
#[test]
fn hand_written_enrichment_is_accepted_by_enrich() {
    struct Computed(i64);

    impl observed::enrichment::Enrichment for Computed {
        fn into_entries(self) -> Vec<EnrichmentEntry> {
            (0..self.0)
                .map(|i| EnrichmentEntry::unclassified(if i == 0 { "first" } else { "second" }, i))
                .collect()
        }
    }
...
#[test]
fn entry_order_is_preserved_and_duplicate_keys_are_kept() {
    #[derive(Debug, Enrichment)]
    struct Ordered {
        #[unredacted]
        first: i64,
    ...
    // A nested scope may re-use a key: both entries survive into the chain, and
    // the inner one is visited last.
    #[derive(Debug, Enrichment)]
    struct Tenant {
        #[unredacted]
        tenant: i64,
    ...
    (|| {
        ...
        .enrich(&sink, Tenant { tenant: 2 })();
    })
    .enrich(&sink, Tenant { tenant: 1 })();

    let captured = processor.single_event();
    let tenants: Vec<_> = captured
        .dimensions()
        .into_iter()
        .filter(|(key, _)| key == "tenant")
        .map(|(_, value)| observed_helpers::metric_number_of(&value))
        .collect();
