...
fn value_conversions_and_accessors() {
    ...
    assert_eq!(observed_helpers::metric_number_of(&numeric), Some(3.0));

    let float = Value::from(1.5_f64);
    assert_eq!(observed_helpers::metric_number_of(&float), Some(1.5));

    // Non-numeric values yield no metric number.
    let boolean = Value::from(true);
    assert_eq!(observed_helpers::metric_number_of(&boolean), None);
}
