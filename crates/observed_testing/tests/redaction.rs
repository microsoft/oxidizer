...
use observed::{Enrichment, Severity, Sink, emit, event};
...
// ---------------------------------------------------------------------------
// `#[data_class(...)]` borrows rather than clones
// ---------------------------------------------------------------------------

/// A classified payload that deliberately does not implement `Clone`.
///
/// `Sensitive<T>` is `RedactedDisplay` whenever `T: Display`, and the generated
/// redaction only borrows the temporary `Sensitive`, so the
/// generated `#[data_class(...)]` getter never needs to own the value. Omitting
/// `Clone` here turns that into a compile-time guarantee: if the getter ever
/// goes back to cloning the field, this event stops compiling.
...
#[event("data_class.borrowed")]
#[info]
struct BorrowedClassified {
    #[data_class(TestTaxonomy::Pii.data_class())]
    payload: NotClone,

    #[data_class(TestTaxonomy::Pii.data_class())]
    optional: Option<NotClone>,
}

#[test]
fn data_class_fields_are_redacted_without_being_cloned() {
    let processor = MockProcessor::with_redaction_engine(engine_replace_stars());
    let sink = Sink::new("borrowed", vec![Arc::new(processor.clone())], tick::SimpleClock::new_frozen());

    emit!(
        sink,
        BorrowedClassified {
            payload: NotClone("abcd"),
            optional: Some(NotClone("xy")),
        }
    );
