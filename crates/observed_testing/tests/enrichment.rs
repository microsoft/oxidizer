...
// ---------------------------------------------------------------------------
// Composite sink: duplicate and overlapping leaves
// ---------------------------------------------------------------------------

/// Cloned leaves share one enrichment slot, so listing a leaf twice would push
/// onto and later restore the same thread-local chain twice, stranding a live
/// enrichment node after the scope ends. There is no case where that is what
/// the caller wanted, so construction rejects it outright.
#[test]
#[should_panic(expected = "received the same leaf twice")]
fn composite_rejects_duplicate_children() {
    static APP: SinkId = SinkId::new("duplicate_child_app");

    let (app, _app_proc) = test_emitter(APP);
    let _ = Sink::composite([app.clone(), app]);
}

/// The same applies when a leaf is reachable through more than one nested
/// composite rather than listed twice directly.
#[test]
#[should_panic(expected = "received the same leaf twice")]
fn composite_rejects_overlapping_nested_composites() {
    static APP: SinkId = SinkId::new("overlap_app");
    static AUDIT: SinkId = SinkId::new("overlap_audit");

    let (app, _app_proc) = test_emitter(APP);
    let (audit, _audit_proc) = test_emitter(AUDIT);
    let inner = Sink::composite([app.clone(), audit]);

    let _ = Sink::composite([inner, Sink::composite([app])]);
}

/// A composite over distinct leaves captures each one's chain, so applying and
/// dropping the transfer on another thread restores it to exactly its prior
/// state.
#[test]
fn transfer_context_captures_every_leaf() {
    static APP: SinkId = SinkId::new("transfer_app");
    static AUDIT: SinkId = SinkId::new("transfer_audit");
    ...
    .enrich(&composite, TenantContext { tenant: PublicI64(7) })();
