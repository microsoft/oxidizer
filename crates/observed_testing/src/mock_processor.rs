...
impl CapturedEvent {
    ...
    /// Returns the captured event severity, or `None` when the event carries no
    /// log signal.
    ///
    /// A metric-only or signal-less event has no severity at all, so this stays
    /// `None` rather than reporting an invented one - which is what lets a test
    /// tell such an event apart from an explicit `Info` log.
    #[must_use]
    ...
    fn from_event_view(event: &EventView<'_>, engine: &data_privacy::RedactionEngine) -> Self {
        ...
        let _ = event.visit_fields(&mut |desc, get_value| {
            ...
            if let Some(metric) = desc.metric()
                ...
                && let Some(number) = observed_helpers::metric_number_of(&value)
            {
...
struct MockProcessorInner {
    ...
    /// Processor name and message reported by `flush`. When `None`, `flush`
    /// succeeds.
    flush_error: Option<(Cow<'static, str>, String)>,
}
...
impl MockProcessor {
    ...
    /// Creates a mock processor whose [`flush`](EventProcessor::flush) always
    /// fails, reporting `processor` as the failing processor and `message` as
    /// the cause.
    ///
    /// Events are still captured, so a failing flush can be combined with the
    /// usual event assertions.
    #[must_use]
    pub fn with_flush_error(processor: impl Into<Cow<'static, str>>, message: impl Into<String>) -> Self {
        Self {
            inner: Arc::new(MockProcessorInner {
                events: Mutex::new(Vec::new()),
                redaction_engine: passthrough_redaction_engine(),
                interest_filter: None,
                flush_error: Some((processor.into(), message.into())),
            }),
        }
    }
...
impl EventProcessor for MockProcessor {
    ...
    fn flush(&self) -> Result<(), observed::FlushError> {
        match &self.inner.flush_error {
            Some((processor, message)) => Err(observed::FlushError::new(processor.clone(), message.clone())),
            None => Ok(()),
        }
    }
}
...
impl ExpectedEvent {
    ...
    /// Creates a new expected event that carries **no severity**, as a
    /// metric-only or signal-less event does.
    ///
    /// Pair with [`metric`](Self::metric) to describe a metric-only event.
    #[must_use]
    pub fn without_severity(name: impl Into<String>) -> Self {
        Self {
            name: name.into(),
            severity: None,
            body: None,
...
/// for the attributes you want to verify. All specified attributes are checked
/// against the `DESCRIPTION` constant generated by `#[event(...)]`.
///
/// # Example
///
/// ```
/// use observed::metadata::InstrumentKind;
/// use observed::{Event, Severity, event};
/// use observed_testing::ExpectedEventDescription;
///
/// # #[event("http.request")]
/// # #[info("Request")]
/// # #[counter(name = "http.requests")]
/// # struct HttpRequest;
/// assert_eq!(
