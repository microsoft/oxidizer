...
//! wall-clock `internity_compare` suite.
//!
...
//! subgroup under the `internity_compare/<op>` Criterion group.
...
//!
//! Run with `cargo bench --bench internity_compare_cg` (requires
//! `gungraun-runner` + Valgrind).
...
fn pop_internity_sym() -> (LocalLexicon, Sym) {
...
fn insert_internity(mut it: LocalLexicon) -> (LocalLexicon, Sym) {
...
#[library_benchmark]
#[bench::case(pop_internity())]
fn reuse_internity(mut it: LocalLexicon) -> (LocalLexicon, Sym) {
...
fn lookup_internity(input: (LocalLexicon, Sym)) -> (LocalLexicon, usize) {
    let (it, sym) = input;
...
fn pop_string_cache() -> StringCacheState {
    let mut fillers = Vec::with_capacity(N as usize);
    for i in 0..N {
    ...
    debug_assert_eq!(state.fillers.len(), N as usize);
    state
}

#[library_benchmark]
#[bench::case(pop_string_cache())]
fn insert_string_cache(state: StringCacheState) -> (StringCacheState, string_cache::DefaultAtom) {
...
#[library_benchmark]
#[bench::case(pop_string_cache())]
fn reuse_string_cache(state: StringCacheState) -> (StringCacheState, string_cache::DefaultAtom) {
    let a = black_box(string_cache::DefaultAtom::from(black_box(KEY)));
    // Return the populated state and the result so refcounting drops are not measured.
    (state, a)
}

#[library_benchmark]
#[bench::case(pop_string_cache())]
fn lookup_string_cache(state: StringCacheState) -> (StringCacheState, usize) {
    let n = black_box((state.key.as_ref() as &str).len());
    (state, n)
}
