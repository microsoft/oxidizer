...
//!   this is the *frozen* read form, which sheds the string→handle map; for lasso
//!   both forms are reported separately (see below); for the interners without a
//!   frozen form it is the same filled interner.
//!
//! lasso is measured as two rows so each maps to the benchmark that shares its object
//! state: `lasso (live Rodeo)` matches the single-threaded `lookup` benchmark (which
//! resolves against a live `Rodeo`), and `lasso (RodeoResolver)` matches the
//! `lookup-concurrent` `lasso-resolver` benchmark (which resolves against the frozen
//! `RodeoResolver`).
//!
//! Run with `cargo bench --bench internity_mem`. The tracking allocator makes this binary
...
use internity::{LocalLexicon, ThreadedLexicon};
...
fn main() {
    ...
    two(
        "internity  LocalLexicon",
        || fill_lexicon(&corpus),
        || fill_lexicon(&corpus).freeze(),
    );
    two(
    ...
    two("lasso (live Rodeo)", || fill_lasso(&corpus), || fill_lasso(&corpus));
    two(
        "lasso (RodeoResolver)",
        || fill_lasso(&corpus),
        || fill_lasso(&corpus).into_resolver(),
    );
    ...
    println!(
        "\nNote: internity `lookup` is the frozen read form, which drops the \
         string→handle map.\n      lasso is reported as two rows: `live Rodeo` \
         (matches the single-threaded `lookup`\n      benchmark) and `RodeoResolver` \
         (matches the `lookup-concurrent` `lasso-resolver`\n      benchmark). \
...
fn fill_lexicon(corpus: &[String]) -> LocalLexicon {
