...
//! dense 1-based index for [`LocalLexicon`](crate::LocalLexicon) — and only decodes its own
...
/// a different interner is range-checked and never causes undefined behavior:
/// [`LocalLexicon::try_resolve`](crate::LocalLexicon::try_resolve) returns `None`
/// when the numeric handle is out of range, but an in-range foreign handle can
/// resolve to an unrelated string.
...
impl Sym {
    ...
    /// Packs a 0-based dense index into a `Sym` (single-threaded [`LocalLexicon`](crate::LocalLexicon)).
    ///
    ...
    /// The 0-based dense index (single-threaded [`LocalLexicon`](crate::LocalLexicon)).
    #[inline]
    ...
    /// [`LocalLexicon::try_resolve`](crate::LocalLexicon::try_resolve) to resolve safely.
    #[inline]
    #[must_use]
