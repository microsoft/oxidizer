...
/// The trace should be provided without the `(at file:line)` part.
...
macro_rules! assert_trace {
    ($error:expr, $expected_trace:expr) => {{
        ...
        // Expected trace without location suffix
        let expected = format!("> {}", $expected_trace);

        assert!(
            normalized_lines.iter().any(|line| line == &expected),
            "Expected trace not found.\nExpected: {expected}\nActual error:\n{error_display}"
        );
    }};
}
