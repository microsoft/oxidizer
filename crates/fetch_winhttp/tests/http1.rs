...
fn small_and_large_get_and_post_round_trips() {
    ...
    let small = futures::executor::block_on(test_client.client.get(server.url("/small")).fetch_text_body()).expect("small GET succeeds");
    let large_response =
        futures::executor::block_on(test_client.client.get(server.url("/large")).fetch_text_body()).expect("large GET succeeds");
    ...
    .expect("known POST succeeds");
    let large_upload = futures::executor::block_on(test_client.client.post(server.url("/large-upload")).text(&large).fetch_text_body())
        .expect("large POST succeeds");
...
fn unknown_length_upload_is_streamed_before_the_response() {
    ...
    let response = futures::executor::block_on(test_client.client.post(server.url("/stream")).body(body).fetch_text_body())
        .expect("unknown-length POST succeeds");
...
fn request_trailers_fail_explicitly() {
    ...
    let request_body = StreamBody::new(futures::stream::iter([
        ...
        Ok(Frame::trailers(HeaderMap::from_iter([(
            "x-request-trailer".parse().expect("valid name"),
    ...
    .expect_err("request trailers are rejected");
...
fn compression_redirects_and_cookies_follow_transport_policy() {
    ...
    let server = TestServer::http([
        ...
        ResponsePlan::status(StatusCode::FOUND).header(
            LOCATION,
            HeaderValue::from_str(&redirect_target).expect("destination URL is a valid header value"),
        ),
    ...
    let gzip = futures::executor::block_on(test_client.client.get(server.url("/gzip")).fetch_text_body()).expect("gzip response succeeds");
    let deflate =
        futures::executor::block_on(test_client.client.get(server.url("/deflate")).fetch_text_body()).expect("deflate response succeeds");
    let brotli = futures::executor::block_on(test_client.client.get(server.url("/brotli")).fetch()).expect("brotli response succeeds");
    let zstd = futures::executor::block_on(test_client.client.get(server.url("/zstd")).fetch()).expect("zstd response succeeds");
    let redirect =
        futures::executor::block_on(test_client.client.get(server.url("/redirect")).fetch()).expect("redirect response is surfaced");
    futures::executor::block_on(test_client.client.get(server.url("/set-cookie")).fetch_text_body())
        .expect("cookie-setting response succeeds");
    futures::executor::block_on(test_client.client.get(server.url("/check-cookie")).fetch_text_body())
        .expect("subsequent request succeeds");

    assert_eq!(gzip, "hello");
    assert_eq!(deflate, "hello");
    assert_eq!(brotli.headers().get(CONTENT_ENCODING).expect("brotli encoding is preserved"), "br");
    assert_eq!(zstd.headers().get(CONTENT_ENCODING).expect("zstd encoding is preserved"), "zstd");
    assert_eq!(
        futures::executor::block_on(brotli.into_body().into_bytes()).expect("brotli body reads"),
        BytesView::copied_from_slice(b"opaque brotli", &test_client.body_builder)
    );
    assert_eq!(
        futures::executor::block_on(zstd.into_body().into_bytes()).expect("zstd body reads"),
        BytesView::copied_from_slice(b"opaque zstd", &test_client.body_builder)
    );
    assert_eq!(redirect.status(), StatusCode::FOUND);
    assert_eq!(
        redirect.headers().get(LOCATION).expect("redirect location is preserved"),
...
fn authentication_challenges_are_surfaced_without_automatic_retry() {
    ...
    let response = futures::executor::block_on(test_client.client.get(server.url("/authentication")).fetch()).expect("401 is a response");
    ...
    assert_eq!(
        response
            ...
            .expect("authentication challenge is preserved"),
        "Basic realm=\"test\""
    );
...
fn full_fetch_pipeline_uses_the_winhttp_transport() {
    ...
    let response =
        futures::executor::block_on(client.get(server.url("/full-pipeline")).fetch_text_body()).expect("the full fetch pipeline succeeds");
...
fn clients_reuse_only_their_own_connection_pools() {
    ...
    futures::executor::block_on(shared.client.get(shared_server.url("/one")).fetch_text_body()).expect("first request succeeds");
    futures::executor::block_on(shared_clone.get(shared_server.url("/two")).fetch_text_body()).expect("clone request succeeds");
    ...
    futures::executor::block_on(first.client.get(isolated_server.url("/one")).fetch_text_body()).expect("first client succeeds");
    futures::executor::block_on(second.client.get(isolated_server.url("/two")).fetch_text_body()).expect("second client succeeds");
    ...
    futures::executor::block_on(first.get(builder_server.url("/one")).fetch_text_body()).expect("first build succeeds");
    futures::executor::block_on(second.get(builder_server.url("/two")).fetch_text_body()).expect("second build succeeds");
    ...
    for path in ["/one", "/two", "/three"] {
        futures::executor::block_on(client.get(pools_server.url(path)).fetch_text_body()).expect("pool request succeeds");
    }
...
fn dropping_a_pending_download_cancels_it_without_blocking_later_requests() {
    ...
    let test_client = client(&[Version::HTTP_11], WinHttpTlsConfig::default());
    let response =
        futures::executor::block_on(test_client.client.get(server.url("/stall")).fetch()).expect("stalled response headers arrive");
    let mut stream = response.into_body().into_stream();
    let first = futures::executor::block_on(stream.try_next())
        .expect("first response chunk succeeds")
        .expect("first response chunk is present");
    ...
    let response = futures::executor::block_on(test_client.client.get(server.url("/after-cancellation")).fetch_text_body())
        .expect("request after cancellation succeeds");
...
fn mixed_completion_cancellation_and_body_drops_remain_usable() {
    ...
    for index in 0..18 {
        let response = futures::executor::block_on(test_client.client.get(server.url(&format!("/soak/{index}"))).fetch())
            .expect("soak response headers arrive");
        match index % 3 {
            0 => {
                let body = futures::executor::block_on(response.into_body().into_text()).expect("completed body reads");
                assert_eq!(body, format!("complete {index}"));
            }
            1 => {
                let mut stream = response.into_body().into_stream();
                let first = futures::executor::block_on(stream.try_next())
                    .expect("partial body read succeeds")
                    .expect("partial body is present");
    ...
    let final_response = futures::executor::block_on(test_client.client.get(server.url("/final")).fetch_text_body())
        .expect("the client remains usable after the soak");
