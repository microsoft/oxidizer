...
async fn default_len_returns_none() {
    ...
    assert!(cache.len().await.expect("len should return Ok").is_none());
}
...
async fn default_is_empty_returns_none_when_len_is_none() {
    ...
    assert!(cache.is_empty().await.expect("len should return Ok").is_none());
}
...
async fn dynamic_cache_len() {
    ...
    assert_eq!(dynamic.len().await.expect("len should return Ok"), Some(0));
    dynamic.insert("key".to_string(), CacheEntry::new(42)).await.unwrap();
    assert_eq!(dynamic.len().await.expect("len should return Ok"), Some(1));
}
