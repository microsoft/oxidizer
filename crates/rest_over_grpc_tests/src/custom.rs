...
use pb::{
    CreateShelfRequest, Genre, GetShelfRequest, GetTreeRequest, GetTreeResponse, ListShelvesByGenreRequest, ListShelvesRequest,
    ListShelvesResponse, SearchShelvesRequest, Shelf, UpdateShelfRequest, UpdateShelfResponse,
};
...
impl pb::Library for InMemoryLibrary {
    ...
    async fn search_shelves(&self, request: SearchShelvesRequest, _cx: &mut Context) -> Result<SearchShelvesRequest, Status> {
        Ok(request)
    }

    async fn update_shelf(&self, request: UpdateShelfRequest, _cx: &mut Context) -> Result<UpdateShelfResponse, Status> {
        let mut updated = request.shelf.ok_or_else(|| Status::invalid_argument("shelf is required"))?;
        updated.name = format!("shelves/{}", request.id);
        Ok(UpdateShelfResponse {
            shelf: Some(updated),
            result_code: i32::from(request.force),
        })
    }

    async fn stream_shelf_themes(&self, request: ListShelvesRequest, cx: &mut Context) -> Result<ResponseStream<Shelf>, Status> {
        self.stream_shelves(request, cx).await
    }

    async fn get_tree(&self, request: GetTreeRequest, _cx: &mut Context) -> Result<GetTreeResponse, Status> {
        Ok(GetTreeResponse { path: request.path })
    }
}
