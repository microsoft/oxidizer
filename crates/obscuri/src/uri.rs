...
use data_privacy::{
    Classified, DataClass, RedactedDebug, RedactedDisplay, RedactedToString, RedactionEngine,
    Sensitive,
};
...
impl Uri {
    ...
    /// let path_and_query = PathAndQuery::from_static("/path?query=1");
    /// let uri: Uri =
    ///     Uri::with_base_and_path(Some(base_uri.into()), Some(path_and_query.into()));
    /// ```
    #[must_use]
    ...
    pub fn to_path_and_query(&self) -> Result<Option<PathAndQuery>, ValidationError> {
        self.path_and_query
            .as_ref()
            .map(TargetPathAndQuery::to_path_and_query)
            .transpose()
    }
    ...
    pub fn to_string(&self) -> Sensitive<String> {
        ...
        match self
            .path_and_query
            .as_ref()
            .map(TargetPathAndQuery::to_uri_string)
        {
...
impl RedactedDisplay for Uri {
    ...
    fn fmt(&self, engine: &RedactionEngine, f: &mut Formatter) -> fmt::Result {
        self.base_uri.as_ref().map_or(Ok(()), |base_uri| {
            f.write_str(base_uri.to_string().as_str())
        })?;
...
impl TryFrom<http::Uri> for Uri {
    ...
    fn try_from(uri: http::Uri) -> Result<Self, Self::Error> {
        ...
        let path_and_query = parts.path_and_query.map(|pq| {
            TargetPathAndQuery::PathAndQuery(Sensitive::<PathAndQuery>::new(
                pq,
...
impl TargetPathAndQuery {
    ...
    pub fn template(&self) -> Cow<'static, str> {
        match self {
            Self::PathAndQuery(classified_pq) => {
                Cow::Owned(classified_pq.clone().declassify_ref().to_string())
            }
    ...
    pub fn to_uri_string_redacted(&self, redaction_engine: &RedactionEngine) -> String {
        match self {
            ...
            Self::TemplatedPathAndQuery(templated) => {
                templated.deref().to_redacted_string(redaction_engine)
            }
        }
    }
...
impl Debug for TargetPathAndQuery {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TemplatedPathAndQuery(templated) => f
                .debug_tuple("TemplatedPathAndQuery")
                .field(templated)
                .finish(),
        }
    }
...
impl TryFrom<Uri> for TargetPathAndQuery {
    ...
    fn try_from(uri: Uri) -> Result<Self, Self::Error> {
        uri.to_path_and_query()?
            ...
            .ok_or_else(|| {
                ValidationError::caused_by("URI does not have a path and query component")
            })
    }
}
...
impl TryFrom<Uri> for PathAndQuery {
    type Error = ValidationError;
    fn try_from(uri: Uri) -> Result<Self, Self::Error> {
        uri.to_path_and_query()?.ok_or_else(|| {
            ValidationError::caused_by("URI does not have a path and query component")
        })
    }
}
...
mod tests {
    ...
    fn test_uri_from_http_uri() {
        ...
        let uri: Uri = http_uri
            .clone()
            .try_into()
            .expect("Failed to convert http::Uri to Uri");
        assert_eq!(uri.to_string().declassify_ref(), uri_str);

        let target_hyper_uri: http::Uri =
            uri.try_into().expect("Failed to convert Uri to http::Uri");
    ...
    fn test_uri_into_http_uri() {
        ...
        let uri: Uri = Uri::default()
            .base_uri(base_uri)
            .path_and_query(path_and_query_with_slash.clone());
        ...
        let uri: Uri = Uri::default()
            ...
            .path_and_query(path_and_query_with_slash);
        let http_uri: http::Uri = uri.try_into().expect("Failed to convert Uri to http::Uri");
        assert_eq!(
            http_uri.to_string(),
        ...
        let uri: Uri = Uri::default()
            .base_uri(base_uri)
            .path_and_query(path_and_query_without_slash);
        let http_uri: http::Uri = uri.try_into().expect("Failed to convert Uri to http::Uri");
        assert_eq!(
            http_uri.to_string(),
            "https://example.com/foo/path?query=1",
    ...
    fn test_path_and_query_template() {
        ...
        assert_eq!(
            target_path_and_query.template(),
            "/path/to/resource?query=param"
        );
        assert_eq!(
            target_path_and_query.to_uri_string(),
            "/path/to/resource?query=param"
        );
        assert_eq!(
            target_path_and_query.to_path_and_query().unwrap(),
            path_and_query
        );
        assert_eq!(
    ...
    fn redact_path_and_query_uri() {
        let insensitive_paq = |paq: &'static str| {
            TargetPathAndQuery::from_path_and_query(PathAndQuery::from_static(paq))
        };
