...
mod tests {
    ...
    fn expand_of(item: Item) -> String {
        expand(item).to_string()
    }

    #[test]
    fn a_named_struct_gains_a_named_core() {
        let expanded = expand_of(parse_quote! {
            struct T { path: String }
        });

        assert!(expanded.contains("derive (:: ohno :: Error)"), "{expanded}");
        assert!(expanded.contains("ohno_core : :: ohno :: OhnoCore"), "{expanded}");
        assert!(expanded.contains("ohno::generated-core@7f3d9c2a"), "{expanded}");
    }
    ...
    fn a_colliding_name_is_numbered() {
        ...
        assert!(expanded.contains("ohno_core_2 : :: ohno :: OhnoCore"), "{expanded}");
    }
    ...
    fn a_tuple_struct_gains_a_trailing_core() {
        ...
        assert!(expanded.contains("String"), "{expanded}");
        assert!(expanded.contains(":: ohno :: OhnoCore"), "{expanded}");
    }
    ...
    fn other_attributes_and_docs_survive() {
        ...
        assert!(expanded.contains("derive (Clone)"), "{expanded}");
    ...
    fn a_marked_field_is_rejected() {
        ...
        assert!(expanded.contains("no field may be marked"), "{expanded}");
        assert!(!expanded.contains("struct T"), "{expanded}");
    }
    ...
    fn a_hand_written_reserved_marker_is_rejected() {
        ...
        assert!(expanded.contains("reserved for"), "{expanded}");
    }
    ...
    fn no_constructors_is_rejected() {
        ...
        assert!(expanded.contains("not supported under"), "{expanded}");
    }
    ...
    fn a_non_struct_is_rejected() {
        ...
        assert!(expanded.contains("supports structs only"), "{expanded}");
    }
