...
/// - `#[no_constructors]` - Disable automatic constructor generation. The `new()` and `caused_by()`
///   constructors generated by default are `pub(crate)` even when the error type is `pub`, so they
///   are usable only inside the crate that defines the error; write your own constructor to expose
///   one to other crates. Rejected under `#[ohno::error]`, which adds the `OhnoCore` field a
///   hand-written constructor would have to name
