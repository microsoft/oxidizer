...
function Write-Changelog {
    ...
    # Get commits since the latest tag (unreleased commits) that touched this package.
    $range = if ($latestTag) { "$latestTag..HEAD" } else { "HEAD" }
    $commitHashes = @(Invoke-Git -Arguments @('log', $range, '--pretty=format:%H', '--', $packageFolder) | Where-Object { $_ })
    ...
    foreach ($hash in $commitHashes) {
        $changedFiles = @(Invoke-Git -Arguments @('show', $hash, '--name-only', '--pretty=format:', '--', $packageFolder) | Where-Object { $_ })
        ...
        if ($hasMeaningfulChange) {
            $rawCommits += Invoke-Git -Arguments @('show', '-s', '--pretty=format:%s', $hash)
        }
    }
