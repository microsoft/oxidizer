@{
    ...
    Description = 'Stable (>=1.x.y) workspace: the menu offers all 5 options because non-breaking (option 4) and patch (option 5) produce distinct numeric outcomes. User picks option 5 on upstream; verifies upstream ends at 1.2.4 (a patch change), proving the patch path is reachable end-to-end and is NOT a synonym for option 4 (which would yield 1.3.0). Counter-balances the 0.x-only synthetic-workspace presets, which collapse 4 and 5 into the same numeric increment.'
